...
fun JUnit.configureForKotlin(xmx: String = "1600m") {
    vmParameters = listOf("-ea", "-XX:+HeapDumpOnOutOfMemoryError", "-Xmx$xmx", "-XX:+UseCodeCacheFlushing",
            ...
            "-Didea.use.native.fs.for.win=false", "-Djps.kotlin.home=${ideaPluginDir.absolutePath}", "-Duse.jps=true",
            "-Djava.awt.headless=true").filterNotNull().joinToString(" ")

    envs = mapOf("NO_FS_ROOTS_ACCESS_CHECK" to "true",
...
if (kotlinBuildProperties.isInJpsBuildIdeaSync) {
    allprojects {
        ...
        afterEvaluate {
            ...
            listOf("testCompile", "testCompileOnly", "testRuntime", "testRuntimeOnly").forEach { configurationName ->
                ...
                val dependencyProjects =
                    configuration?.dependencies?.mapNotNull { (it as? ProjectDependency)?.dependencyProject }
...
val jarArtifactProjects = listOf("kotlin-compiler-client-embeddable", "kotlin-compiler", "kotlin-daemon-client",
        "kotlin-imports-dumper-compiler-plugin", "kotlin-jps-plugin", "kotlin-main-kts", "kotlin-reflect")
...
fun NamedDomainObjectContainer<TopLevelArtifact>.kotlinReflectJar() = jarFromProject(project(":kotlin-reflect"))
...
fun NamedDomainObjectContainer<TopLevelArtifact>.jarFromProject(project: Project, name: String? = null,
        configureAction: RecursiveArtifact.() -> Unit = {}) {
...
fun RecursiveArtifact.archiveFromProject(project: Project, name: String? = null,
        configureAction: RecursiveArtifact.() -> Unit = {}) {
    val jarName = name ?: project.name + ".jar"
    archive(jarName) {
...
fun RecursiveArtifact.jarContentsFromConfiguration(configuration: Configuration) {
    ...
    resolvedArtifacts.filter { it.id.componentIdentifier is ModuleComponentIdentifier }.map { it.file }
            .forEach { extractedDirectory(it) }

    resolvedArtifacts.map { it.id.componentIdentifier }.filterIsInstance<ProjectComponentIdentifier>().forEach {
...
fun RecursiveArtifact.sourceJarsFromConfiguration(configuration: Configuration, renamer: (String) -> String = { it }) {
    ...
    resolvedArtifacts.map { it.id.componentIdentifier }.filterIsInstance<ProjectComponentIdentifier>().forEach {
                ...
                archive(renamed) {
                    project(it.projectPath).mainSourceSet.allSource.sourceDirectories.forEach { sourceDirectory ->
...
fun RecursiveArtifact.jarsFromExternalModules(resolvedArtifacts: Iterable<ResolvedArtifact>,
        renamer: (String) -> String = { it }) {
    // Use output-file-name property when fixed https://github.com/JetBrains/gradle-idea-ext-plugin/issues/63
    resolvedArtifacts.filter { it.id.componentIdentifier is ModuleComponentIdentifier }.forEach {
