...
private fun Project.ideModuleVersion(forIde: Boolean) = when (IdeVersionConfigurator.currentIde.kind) {
    ...
    Ide.Kind.IntelliJ -> {
        if (forIde) {
            intellijSdkVersionForIde() ?: error("Please specify 'attachedIntellijVersion' in your local.properties")
        } else {
...
fun RepositoryHandler.kotlinBuildLocalRepo(project: Project): IvyArtifactRepository = ivy {
    ...
    patternLayout {
        ...
        artifact(
                "[organisation]/${project.ideModuleName()}/[revision]/artifacts/plugins/[module]/lib/[artifact](-[classifier]).[ext]") // bundled plugins
...
@JvmOverloads fun Project.intellijDep(module: String? = null, forIde: Boolean = false) =
...
object IntellijRootUtils {
    ...
    fun getIntellijRootDir(project: Project): File = with(project.rootProject) {
        return File(getRepositoryRootDir(this), "${ideModuleName()}/${ideModuleVersion(forIde = false)}/artifacts")
    }
}
