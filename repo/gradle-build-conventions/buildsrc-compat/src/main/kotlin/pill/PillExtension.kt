...
open class PillExtension {
    ...
    @Suppress("unused") fun Project.excludedDirs(vararg dirs: String) {
        excludedDirs = excludedDirs + dirs.map { File(projectDir, it) }
    }

    @Suppress("unused") fun serialize() =
        mapOf<String, Any?>("variant" to variant?.name, "excludedDirs" to excludedDirs)
}