...
fun Project.checkExpectedGradlePropertyValues() {
    ...
    val issues = expectKeys.mapNotNull { expectKey ->
        ...
        if (expectedValue != actualValue) return@mapNotNull UnexpectedPropertyValue(actualKey, expectedValue,
                actualValue)
...
private sealed class GradlePropertyIssue {
    data class UnexpectedPropertyValue(val key: String, val expectedValue: String, val actualValue: String) :
        GradlePropertyIssue()

    data class MissingProperty(val key: String, val expectedValue: String) : GradlePropertyIssue()
}
