...
private val allowImplicitDependOnNpmForTasks =
    setOf("kotlinUpgradeYarnLock", "compileTestKotlinJs", "compileTestKotlinWasm")
...
private fun tasksGraphString(taskGraph: TaskExecutionGraph, ident: String = "",
        nextIdent: (String) -> String = { "$it|-" }): String {
    return buildString {
...
val Project.checkYarnAndNPMSuppressed: Action<TaskExecutionGraph>
    get() {
        return Action<TaskExecutionGraph> {
            val disableNpmYarnCheck =
                providers.gradleProperty("kotlin.build.disable.npmyarn.suppress.check").orNull?.toBoolean() ?: false
            ...
            if (npmYarnTasks.isNotEmpty()) {
                if (allowedTask.isEmpty()) {
                    error("$npmYarnTasks tasks shouldn't be present in the task graph: $npmYarnTasks " + "as $allowImplicitDependOnNpmForTasks tasks were not activated\n" + "Graph:\n${
