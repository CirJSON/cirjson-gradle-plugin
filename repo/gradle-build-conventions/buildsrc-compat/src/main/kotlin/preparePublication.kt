...
fun Project.preparePublication() {
    tasks.register("preparePublication") {
        ...
        val repositoryProviders =
            mapOf<String?, String?>("sonatype-nexus-staging" to "sonatype", "sonatype-nexus-snapshots" to "sonatype")
        ...
        val deployFolder = properties["deployRepoFolder"]?.toString()?.let { "file://${rootProject.buildDir}/$it" }
        ...
        val isDeployStagingRepoGenerationRequired: Boolean by extra(
                isSonatypeRelease && deployUrlFromParameters == null)
        ...
        val password: String? by extra(properties["deployRepoPassword"]?.toString()
            ?: properties["kotlin.${repoProvider}.password"]?.toString())
                ...
                HttpClient().use { client ->
                    runBlocking {
                        ...
                        val response = client.post(
                                "https://oss.sonatype.org/service/local/staging/profiles/169b36e205a64e/start") {
                        ...
                        if (response.status.value in 200..299) {
                            ...
                            val repoId = responseText.substringAfter("<stagingRepositoryId>")
