...
private class BuildTimeReporter(private val kotlinCompileClass: Class<*>, private val javaCompileClass: Class<*>,
        private val jarClass: Class<*>, private val proguardClass: Class<*>) :
    ...
    companion object {
        fun configure(gradle: Gradle) {
            ...
            @Suppress("DEPRECATION") gradle.taskGraph.addTaskExecutionListener(reporter)
    ...
    private enum class TaskCategory {
        COMPILING_KOTLIN, COMPILING_JAVA, PROCESSING_JARS, UNCATEGORIZED;

        private fun String.toTitleCase() = if (length <= 1) capitalize() else {
            get(0).uppercase() + substring(1).lowercase()
        }

        fun description(): String = name.replace("_", " ").toTitleCase()
    }
    ...
    @Synchronized override fun beforeExecute(task: Task) {
        taskStartTime[task] = System.nanoTime()
    }

    @Synchronized override fun afterExecute(task: Task, state: TaskState) {
    ...
    private fun taskCategory(task: Task): TaskCategory = when {
        ...
        jarClass.isInstance(task) || proguardClass.isInstance(task) -> TaskCategory.PROCESSING_JARS
        else -> TaskCategory.UNCATEGORIZED
    }

    private fun Double.asShortString() = String.format("%.2f", this)

    @Synchronized private fun report(log: Logger) {
        ...
        for (category in TaskCategory.values()) {
            ...
            log.info(
                    "${category.description()}: ${timeSec.asShortString()}s (${percent.asShortString()}% of total time)")
        }
    }
}
