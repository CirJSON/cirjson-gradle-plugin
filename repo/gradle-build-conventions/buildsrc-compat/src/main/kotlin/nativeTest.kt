...
private enum class TestProperty(shortName: String) {
    ...
    KOTLIN_NATIVE_HOME("nativeHome"), COMPILER_CLASSPATH("compilerClasspath"), CUSTOM_KLIBS("customKlibs"), TEST_TARGET(
            "target"),
    TEST_MODE("mode"), FORCE_STANDALONE("forceStandalone"), COMPILE_ONLY("compileOnly"), OPTIMIZATION_MODE(
            "optimizationMode"),
    USE_THREAD_STATE_CHECKER("useThreadStateChecker"), GC_TYPE("gcType"), GC_SCHEDULER("gcScheduler"), ALLOCATOR(
            "alloc"),
    CACHE_MODE("cacheMode"), EXECUTION_TIMEOUT("executionTimeout"), SANITIZER("sanitizer"), TEAMCITY("teamcity");
...
private class ComputedTestProperties(private val task: Test) {
    ...
    fun Project.computeLazy(property: TestProperty, defaultLazyValue: () -> Lazy<String?>) {
        val gradleValue = readFromGradle(property)
        computedProperties += if (gradleValue != null) ComputedTestProperty.Normal(property.fullName, gradleValue)
        else ComputedTestProperty.Lazy(property.fullName, defaultLazyValue())
    }
...
fun Project.nativeTest(taskName: String, tag: String?, requirePlatformLibs: Boolean = false,
        customDependencies: List<Configuration> = emptyList(),
        customKlibDependencies: List<Configuration> = emptyList()) = projectTest(taskName, jUnitMode = JUnitMode.JUnit5,
    ...
    if (kotlinBuildProperties.isKotlinNativeEnabled) {
        ...
        if (!kotlinBuildProperties.isTeamcityBuild && minOf(
                        kotlinBuildProperties.junit5NumberOfThreadsForParallelExecution ?: 16, availableCpuCores) > 4) {
        ...
        val computedTestProperties = ComputedTestProperties {
            ...
            computeLazy(COMPILER_CLASSPATH) {
                ...
                val kotlinNativeCompilerEmbeddable = if (customNativeHome == null) configurations.detachedConfiguration(
