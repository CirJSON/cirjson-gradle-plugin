...
enum class JdkMajorVersion(val majorVersion: Int, val targetName: String = majorVersion.toString(),
        private val overrideMajorVersion: Int? = null, private val mandatory: Boolean = true) {
    JDK_1_6(6, targetName = "1.6", overrideMajorVersion = 8), JDK_1_7(7, targetName = "1.7",
            overrideMajorVersion = 8),
    JDK_1_8(8, targetName = "1.8"), JDK_9_0(9, overrideMajorVersion = 11), JDK_10_0(10, mandatory = false,
            overrideMajorVersion = 11),
    JDK_11_0(11, mandatory = false), JDK_16_0(16, mandatory = false), JDK_17_0(17, mandatory = false), JDK_21_0(21,
            mandatory = false);
    ...
    val overrideVersion by lazy {
        if (overrideMajorVersion != null) {
            values().firstOrNull() { it.majorVersion == overrideMajorVersion } ?: error(
                    "Can't find the value with majorVersion=$overrideMajorVersion")
        } else {
            null
        }
    }
...
fun Project.configureJvmToolchain(jdkVersion: JdkMajorVersion) {
    @Suppress("NAME_SHADOWING") val jdkVersion = chooseJdk_1_8ForJpsBuild(jdkVersion)
    ...
    plugins.withId("org.jetbrains.kotlin.jvm") {
        ...
        tasks.matching { it.name != "compileJava9Java" && it is JavaCompile }.configureEach {
...
fun KotlinCompile.configureTaskToolchain(jdkVersion: JdkMajorVersion) {
    if (project.shouldOverrideObsoleteJdk(jdkVersion)) {
        kotlinJavaToolchain.toolchain.use(project.getToolchainLauncherFor(
                jdkVersion.overrideVersion ?: error("Substitution version should be defined for override mode")))
        @Suppress("DEPRECATION") kotlinOptions {
            jvmTarget = jdkVersion.targetName
        }
    } else {
        kotlinJavaToolchain.toolchain.use(project.getToolchainLauncherFor(jdkVersion))
    }
}

fun JavaCompile.configureTaskToolchain(jdkVersion: JdkMajorVersion) {
    if (project.shouldOverrideObsoleteJdk(jdkVersion)) {
        javaCompiler.set(project.getToolchainCompilerFor(
                jdkVersion.overrideVersion ?: error("Substitution version should be defined for override mode")))
        targetCompatibility = jdkVersion.targetName
        sourceCompatibility = jdkVersion.targetName
    } else {
...
fun Project.updateJvmTarget(jvmTarget: String) {
    @Suppress("NAME_SHADOWING") val jvmTarget = if (kotlinBuildProperties.isInJpsBuildIdeaSync && jvmTarget == "1.6") {
        "1.8"
    } else {
        jvmTarget
    }
    // Java 9 tasks are exceptions that are configured in configureJava9Compilation
    tasks.withType<KotlinCompile>().matching { it.name != "compileJava9Kotlin" }.configureEach {
                @Suppress("DEPRECATION") kotlinOptions.jvmTarget = jvmTarget
            }

    tasks.withType<JavaCompile>().matching { it.name != "compileJava9Java" }.configureEach {
