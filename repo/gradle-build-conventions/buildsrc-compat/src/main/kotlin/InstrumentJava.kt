...
class InstrumentJava(@Transient val javaInstrumentator: Configuration) : Action<Task> {
    ...
    override fun execute(task: Task) {
        ...
        val dummyInstrumentSrcRelativePath =
            dummyInstrumentSrcDir.relativeTo(task.project.projectDir).path.replace("\\", "/")

        task.doLast {
            task.ant.withGroovyBuilder {
                "taskdef"("name" to "instrumentIdeaExtensions", "classpath" to instrumentatorClasspath,
                        "loaderref" to "java2.loader", "classname" to "com.intellij.ant.InstrumentIdeaExtensions")
            }

            dummyInstrumentSrcDir.mkdirs()

            task.ant.withGroovyBuilder {
                "instrumentIdeaExtensions"("srcdir" to dummyInstrumentSrcRelativePath,
                        "destdir" to task.destinationDirectory.asFile.get(), "classpath" to task.classpath.asPath,
                        "includeantruntime" to false, "instrumentNotNull" to true)
            }
        }
    }
}