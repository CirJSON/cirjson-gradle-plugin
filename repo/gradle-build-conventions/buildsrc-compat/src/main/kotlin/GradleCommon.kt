 ...
 */
enum class GradlePluginVariant(val sourceSetName: String, val minimalSupportedGradleVersion: String,
        val gradleApiVersion: String, val gradleApiJavadocUrl: String) {
    GRADLE_MIN("main", "6.8.3", "6.9", "https://docs.gradle.org/6.9.3/javadoc/"), GRADLE_70("gradle70", "7.0", "7.0",
            "https://docs.gradle.org/7.0.2/javadoc/"),
    GRADLE_71("gradle71", "7.1", "7.1", "https://docs.gradle.org/7.1.1/javadoc/"), GRADLE_74("gradle74", "7.4", "7.4",
            "https://docs.gradle.org/7.4.2/javadoc/"),
    GRADLE_75("gradle75", "7.5", "7.5", "https://docs.gradle.org/7.5.1/javadoc/"), GRADLE_76("gradle76", "7.6", "7.6",
            "https://docs.gradle.org/7.6.1/javadoc/"),
    GRADLE_80("gradle80", "8.0", "8.0", "https://docs.gradle.org/8.0.2/javadoc/"), GRADLE_81("gradle81", "8.1", "8.1",
            "https://docs.gradle.org/8.1.1/javadoc/"),
}
...
private fun Project.fixWiredSourceSetSecondaryVariants(
    ...
    configurations.matching {
                it.name == wireSourceSet.apiElementsConfigurationName || it.name == wireSourceSet.runtimeElementsConfigurationName
            }.configureEach {
                outgoing {
                    variants.maybeCreate("classes").apply {
                        ...
                        (commonSourceSet.output.classesDirs.files + wireSourceSet.output.classesDirs.files).toSet()
                                .forEach {
    ...
    configurations.matching { it.name == wireSourceSet.runtimeElementsConfigurationName }.configureEach {
                outgoing {
                    val resourcesDirectories =
                        listOfNotNull(commonSourceSet.output.resourcesDir, wireSourceSet.output.resourcesDir)
...
fun Project.reconfigureMainSourcesSetForGradlePlugin(
    ...
    sourceSets.named(SourceSet.MAIN_SOURCE_SET_NAME) {
        ...
        if (configurations["default"].attributes.contains(TargetJvmEnvironment.TARGET_JVM_ENVIRONMENT_ATTRIBUTE)) {
            configurations["default"].attributes.attribute(TargetJvmEnvironment.TARGET_JVM_ENVIRONMENT_ATTRIBUTE,
        ...
        listOf(runtimeElementsConfigurationName, apiElementsConfigurationName).map { configurations[it] }
                    ...
                    configurations.create("${originalConfiguration.name}$FIXED_CONFIGURATION_SUFFIX") {
                        ...
                        attributes {
                            originalConfiguration.attributes.keySet()
                                    ...
                                    .forEach { originalAttribute ->
                                        @Suppress("UNCHECKED_CAST") attribute(originalAttribute as Attribute<Any>,
                        ...
                        if (attributes.keySet() != expectedAttributes) {
                            error("Wrong set of attributes:\n" + "  Expected: ${
                                expectedAttributes.joinToString(", ")
                            }\n" + "  Actual: ${
                                attributes.keySet().joinToString(", ") { "${it.name}=${attributes.getAttribute(it)}" }
                            }")
                        }

                        javaComponent.addVariantsFromConfiguration(this) {
                            mapToMavenScope(when (originalConfiguration.name) {
...
fun Project.configureKotlinCompileTasksGradleCompatibility() {
    tasks.withType<KotlinCompile>().configureEach {
        compilerOptions {
            ...
            @Suppress(
                    "DEPRECATION") // we can't use language version greater than 1.5 as minimal supported Gradle embeds Kotlin 1.4
            languageVersion.set(KotlinVersion.KOTLIN_1_5)
            @Suppress(
                    "DEPRECATION") // we can't use api version greater than 1.4 as minimal supported Gradle version uses kotlin-stdlib 1.4
...
fun Project.publishShadowedJar(
    ...
    val shadowJarTask = embeddableCompilerDummyForDependenciesRewriting(
        ...
        setupPublicJar(jarTask.flatMap { it.archiveBaseName }, jarTask.flatMap { it.archiveClassifier })
        addEmbeddedRuntime()
        ...
        exclude {
            ...
            @Suppress("UNNECESSARY_SAFE_CALL", "SAFE_CALL_WILL_CHANGE_NULLABILITY") it.file?.name?.startsWith(
                    "kotlin-compiler-embeddable") ?: false
        }
    }

    // Removing artifact produced by Jar task
    if (sourceSet.name == SourceSet.MAIN_SOURCE_SET_NAME) {
        configurations["${sourceSet.runtimeElementsConfigurationName}$FIXED_CONFIGURATION_SUFFIX"].artifacts.removeAll { true }
        configurations["${sourceSet.apiElementsConfigurationName}$FIXED_CONFIGURATION_SUFFIX"].artifacts.removeAll { true }
    } else {
...
fun Project.addBomCheckTask() {
    val checkBomTask = tasks.register("checkGradlePluginsBom") {
        ...
        doLast {
            ...
            val constraintsLines =
                bomBuildFile.readText().substringAfter("constraints {").substringBefore("}").split("\n")
                        .map { it.trim() }

            val isContainingThisProject = constraintsLines.contains("api(project(\"$projectPath\"))")
...
fun Project.configureDokkaPublication(
    ...
    plugins.withId("org.jetbrains.dokka") {
        ...
        GradlePluginVariant.values().forEach { pluginVariant ->
            ...
            dokkaTask.configure {
                description =
                    "Generates documentation in 'javadoc' format for '${variantSourceSet.javadocTaskName}' variant"
        ...
        if (configurePublishingToKotlinlang) {
            ...
            tasks.register<DokkaTask>("dokkaKotlinlangDocumentation") {
                ...
                dokkaSourceSets {
                    ...
                    pluginsMapConfiguration.put("org.jetbrains.dokka.versioning.VersioningPlugin",
...
private fun GradleExternalDocumentationLinkBuilder.addWorkaroundForElementList(pluginVariant: GradlePluginVariant) {
    if (pluginVariant == GradlePluginVariant.GRADLE_76 || pluginVariant == GradlePluginVariant.GRADLE_80 || pluginVariant == GradlePluginVariant.GRADLE_81) {
