...
val signPublication = !version.toString().contains("-SNAPSHOT") && (project.gradle.startParameter.taskNames.contains(
        "publishPlugins") || signLibraryPublication)
...
if (!kotlinBuildProperties.isInJpsBuildIdeaSync) {
    ...
    val gradle70SourceSet = createGradlePluginVariant(GradlePluginVariant.GRADLE_70, commonSourceSet = commonSourceSet)
    publishShadowedJar(gradle70SourceSet, commonSourceSet)

    // Used for Gradle 7.1+ versions
    val gradle71SourceSet = createGradlePluginVariant(GradlePluginVariant.GRADLE_71, commonSourceSet = commonSourceSet)
    publishShadowedJar(gradle71SourceSet, commonSourceSet)

    // Used for Gradle 7.4+ versions
    val gradle74SourceSet = createGradlePluginVariant(GradlePluginVariant.GRADLE_74, commonSourceSet = commonSourceSet)
    publishShadowedJar(gradle74SourceSet, commonSourceSet)

    // Used for Gradle 7.5+ versions
    val gradle75SourceSet = createGradlePluginVariant(GradlePluginVariant.GRADLE_75, commonSourceSet = commonSourceSet)
    publishShadowedJar(gradle75SourceSet, commonSourceSet)

    // Used for Gradle 7.6+ versions
    val gradle76SourceSet = createGradlePluginVariant(GradlePluginVariant.GRADLE_76, commonSourceSet = commonSourceSet)
    publishShadowedJar(gradle76SourceSet, commonSourceSet)

    // Used for Gradle 8.0+ versions
    val gradle80SourceSet = createGradlePluginVariant(GradlePluginVariant.GRADLE_80, commonSourceSet = commonSourceSet)
    publishShadowedJar(gradle80SourceSet, commonSourceSet)

    // Used for Gradle 8.1+ versions
    val gradle81SourceSet = createGradlePluginVariant(GradlePluginVariant.GRADLE_81, commonSourceSet = commonSourceSet)
