...
fun Project.setPublishableArtifact(jarTask: TaskProvider<out Jar>) {
    addArtifact("runtimeElements", jarTask)
    addArtifact("apiElements", jarTask)
    addArtifact("archives", jarTask)
}

fun removeJarTaskArtifact(jarTask: TaskProvider<out Jar>): Configuration.() -> Unit = {
...
@JvmOverloads fun Jar.addEmbeddedRuntime(embeddedConfigurationName: String = "embedded") {
...
@JvmOverloads fun Project.sourcesJar(body: Jar.() -> Unit = {}): TaskProvider<Jar> {
...
@JvmOverloads fun Jar.addEmbeddedSources(configurationName: String = "embedded") {
    project.configurations.findByName(configurationName)?.let { embedded ->
        val allSources by lazy {
            embedded.resolvedConfiguration.resolvedArtifacts.map { it.id.componentIdentifier }
...
@JvmOverloads fun Project.javadocJar(body: Jar.() -> Unit = {}): TaskProvider<Jar> {
    configure<JavaPluginExtension> {
        withJavadocJar()
    }
...
@JvmOverloads fun Project.publish(moduleMetadata: Boolean = false, sbom: Boolean = true,
    ...
    val publication =
        extensions.findByType<PublishingExtension>()?.publications?.findByName(mainPublicationName) as MavenPublication
...
fun Jar.setupPublicJar(baseName: String, classifier: String = "") =
    setupPublicJar(project.provider { baseName }, project.provider { classifier })

fun Jar.setupPublicJar(baseName: Provider<String>, classifier: Provider<String> = project.provider { "" }) {
...
fun Project.addArtifact(configuration: Configuration, task: Task, artifactRef: Any,
...
fun Project.addArtifact(configurationName: String, task: Task, artifactRef: Any,
        body: ConfigurablePublishArtifact.() -> Unit = {}) =
    addArtifact(configurations.getOrCreate(configurationName), task, artifactRef, body)

fun <T : Task> Project.addArtifact(configurationName: String, task: TaskProvider<T>,
        body: ConfigurablePublishArtifact.() -> Unit = {}): PublishArtifact {
    configurations.maybeCreate(configurationName)
    return artifacts.add(configurationName, task, body)
}

fun <T : Task> Project.addArtifact(configurationName: String, task: TaskProvider<T>, artifactRef: Any,
        body: ConfigurablePublishArtifact.() -> Unit = {}): PublishArtifact {
    configurations.maybeCreate(configurationName)
...
fun Project.configurePublishedComponent(configure: AdhocComponentWithVariants.() -> Unit) =
    (components.findByName(KotlinBuildPublishingPlugin.ADHOC_COMPONENT_NAME) as AdhocComponentWithVariants?)?.apply(
            configure)
