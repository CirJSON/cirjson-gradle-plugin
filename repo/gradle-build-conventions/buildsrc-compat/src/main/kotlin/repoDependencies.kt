...
private val Project.isNightlyIntellij
    get() = rootProject.extra["versions.intellijSdk"].toString().endsWith("SNAPSHOT") && !isEAPIntellij
...
fun Project.commonDependency(group: String, artifact: String, vararg suffixesAndClassifiers: String): String {
    ...
    return "$group:$artifact${artifactSuffixes.joinToString("")}:${
        commonDependencyVersion(group, artifact)
    }${classifiers.joinToString("")}"
}

fun Project.commonDependencyVersion(group: String, artifact: String): String = when {
    ...
    rootProject.extra.has("versions.$group") -> rootProject.extra["versions.$group"]
            ...
            "Neither versions.$artifact nor versions.$group is defined in the root project's extra")
} as String

fun Project.preloadedDeps(vararg artifactBaseNames: String, baseDir: File = File(rootDir, "dependencies"),
        subDir: String? = null, optional: Boolean = false): ConfigurableFileCollection {
    ...
    if (matchingFiles == null || matchingFiles.size < artifactBaseNames.size) {
        throw GradleException(
                "Not all matching artifacts '${artifactBaseNames.joinToString()}' found in the '$dir' " + "(missing: ${
                    artifactBaseNames.filterNot { request ->
                        matchingFiles?.any {
                            it.matchMaybeVersionedArtifact(request)
                        } ?: false
                    }.joinToString()
...
@JvmOverloads fun Project.kotlinStdlib(suffix: String? = null, classifier: String? = null): Any {
    return if (kotlinBuildProperties.useBootstrapStdlib) kotlinDep(listOfNotNull("stdlib", suffix).joinToString("-"),
            bootstrapKotlinVersion, classifier)
    else dependencies.project(listOfNotNull(":kotlin-stdlib", suffix).joinToString("-"), classifier)
}
...
fun DependencyHandler.add(configurationName: String, dependencyNotation: Any,
...
fun DependencyHandler.jpsLikeJarDependency(dependencyNotation: Any, scope: JpsDepScope,
        dependencyConfiguration: (ModuleDependency.() -> Unit)? = null, exported: Boolean = false) {
    when (scope) {
        JpsDepScope.COMPILE -> {
            if (exported) {
...
fun Project.testApiJUnit5(vintageEngine: Boolean = false, runner: Boolean = false, suiteApi: Boolean = false,
    ...
    with(dependencies) {
        ...
        val components =
            mutableListOf("org.junit.platform:junit-platform-commons", "org.junit.platform:junit-platform-launcher")
        if (runner) {
...
private fun String.toMaybeVersionedJarRegex(): Regex {
    ...
    return Regex(
            if (hasJarExtension) escaped else "$escaped(-\\d.*)?\\.jar") // TODO: consider more precise version part of the regex
}

fun Project.firstFromJavaHomeThatExists(vararg paths: String, jdkHome: File = File(
        (this.property("JDK_1_8") ?: this.property("JDK_18") ?: error(
                "Can't find JDK_1_8 property")) as String)): File? =
    paths.map { File(jdkHome, it) }.firstOrNull { it.exists() }.also {
        if (it == null) logger.warn("Cannot find file by paths: ${paths.toList()} in $jdkHome")
    }

fun Project.toolsJarApi(): Any = if (kotlinBuildProperties.isInJpsBuildIdeaSync) toolsJar()
else dependencies.project(":dependencies:tools-jar-api")

fun Project.toolsJar(): FileCollection = files(getToolchainLauncherFor(DEFAULT_JVM_TOOLCHAIN).map {
