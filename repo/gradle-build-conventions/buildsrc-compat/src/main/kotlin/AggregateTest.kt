...
open class AggregateTest : Test() { // Inherit from Test to see test results in IDEA Test viewer
        ...
        project.gradle.taskGraph.whenReady {
            if (allTasks.filterIsInstance<AggregateTest>().isNotEmpty()) {
                ...
                if (!project.gradle.startParameter.taskNames.all { project.tasks.findByPath(it) is AggregateTest }) {
                    logger.warn(
                            "Please, don't use AggregateTest and non-AggregateTest test tasks together. You can get incorrect results.")
                }
            }
        }
    }
    ...
    private fun initPatterns() {
        if (!testPatternFile.exists()) throw GradleException("File with test patterns is not found")
        testPatternFile.readLines().asSequence().filter { it.isNotEmpty() }.forEach { line ->
    ...
    @Override @TaskAction override fun executeTests() {
        // Do nothing
    }
}