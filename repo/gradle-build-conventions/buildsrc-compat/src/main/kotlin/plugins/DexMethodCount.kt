...
abstract class DexMethodCount @Inject constructor(objectFactory: ObjectFactory, layout: ProjectLayout) : DefaultTask() {
    ...
    data class Counts(val total: Int, val totalOwnPackages: Int?, val totalOtherPackages: Int?,
            val byPackage: Map<String, Int>, val byClass: Map<String, Int>)
    ...
    @get:OutputFile
    val detailOutputFile: RegularFileProperty = objectFactory.fileProperty()
            .value(artifactOrArchiveName.flatMap { layout.buildDirectory.file("$it-method-count.txt") })

    @TaskAction fun invoke() {
    ...
    private fun List<DexMethod>.getCounts(): Counts {
        ...
        return Counts(total = this.size, totalOwnPackages = byOwnPackages.first,
                totalOtherPackages = byOwnPackages.second, byPackage = byPackage, byClass = byClass)
    }
    ...
    private fun outputDetails(counts: Counts) {
        detailOutputFile.get().asFile.printWriter().use { writer ->
            ...
            ownPackages.orNull?.let { packages ->
                writer.println(
                        "${counts.totalOwnPackages?.padRight()}\tTotal methods from packages ${packages.joinToString { "$it.*" }}")
...
abstract class DexMethodCountStats : DefaultTask() {
    ...
    @TaskAction private fun printStats() {
        ...
        inputFile.get().asFile.reader().useLines { lines ->
            ...
            ownPackages.map { packages ->
                ...
                logger.lifecycle(
                        "Artifact $artifactOrArchiveName, total methods from packages ${packages.joinToString { "$it.*" }}: $totalOwnPackages")
                logger.lifecycle(
                        "Artifact $artifactOrArchiveName, total methods from other packages: $totalOtherPackages")
...
private val DexMethod.declaringTypeFqn: String
    get() {
