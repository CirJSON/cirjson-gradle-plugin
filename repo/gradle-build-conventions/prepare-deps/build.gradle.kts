...
import org.gradle.internal.os.OperatingSystem //import org.jetbrains.kotlin.gradle.tasks.internal.CleanableStore
...
dependencies {
    if (androidStudioRelease != null) {
        val extension = if (androidStudioOs == "linux") "tar.gz"
        else "zip"
...
fun prepareDeps(intellij: Configuration, intellijCore: Configuration, sources: Configuration,
        jpsStandalone: Configuration, intellijVersion: String) {
    val makeIntellijCore = buildIvyRepositoryTask(intellijCore, customDepsOrg, customDepsRepoDir)

    val makeIntellijAnnotations =
            tasks.register("makeIntellijAnnotations${intellij.name.replaceFirstChar(Char::uppercase)}", Copy::class) {
                dependsOn(makeIntellijCore)

                val intellijCoreRepo =
                        CleanableStore[repoDir.resolve("intellij-core").absolutePath][intellijVersion].use()
                ...
                doLast {
                    writeIvyXml(customDepsOrg, intellijRuntimeAnnotations, intellijVersion, intellijRuntimeAnnotations,
                            targetDir, targetDir, targetDir, allowAnnotations = true)
                }
            }
    ...
    val makeIde = if (androidStudioBuild != null) {
        buildIvyRepositoryTask(androidStudio, customDepsOrg, customDepsRepoDir,
                if (androidStudioOs == "mac") ::skipContentsDirectory
                else ::skipToplevelDirectory)
    } else {
    ...
    tasks.named("build") {
        dependsOn(makeIntellijCore, makeIde, buildJpsStandalone, makeIntellijAnnotations)
    }
}
...
when (kotlinBuildProperties.getOrNull("attachedIntellijVersion")) {
    ...
    "master" -> {} // for intellij/kt-master, intellij maven artifacts are used instead of manual unpacked dependencies
        ...
        val intellijVersionForIde = intellijVersionForIde ?: error(
                "intellijVersionForIde should not be null as attachedIntellijVersion is present")
...
fun buildIvyRepositoryTask(configuration: Configuration, organization: String, repoDirectory: File,
        pathRemap: ((String) -> String)? = null, sources: File? = null): TaskProvider<Task> {
    ...
    fun ResolvedArtifact.moduleDirectory(): File = storeDirectory()[moduleVersion.id.version].use()
    ...
    return tasks.register("buildIvyRepositoryFor${configuration.name.replaceFirstChar(Char::uppercase)}") {
        ...
        doFirst {
            ...
            with(artifact) {
                ...
                writeIvyXml(organization, moduleVersion.id.name, moduleVersion.id.version, moduleVersion.id.name,
                        File(artifactsDirectory, "lib"), File(artifactsDirectory, "lib"), File(moduleDirectory, "ivy"),
                        *listOfNotNull(sources).toTypedArray())

                val pluginsDirectory = File(artifactsDirectory, "plugins")
                if (pluginsDirectory.exists()) {
                    file(File(artifactsDirectory, "plugins")).listFiles { file: File -> file.isDirectory }.forEach {
                            writeIvyXml(organization, it.name, moduleVersion.id.version, it.name, File(it, "lib"),
                                    File(it, "lib"), File(moduleDirectory, "ivy"),
                                    *listOfNotNull(sources).toTypedArray())
                        }
                }
...
fun writeIvyXml(organization: String, moduleName: String, version: String, fileName: String, baseDir: File,
        artifactDir: File, targetDir: File, vararg sourcesJar: File, allowAnnotations: Boolean = false) {
    fun shouldIncludeIntellijJar(jar: File) = jar.isFile && jar.extension == "jar" && !jar.name.startsWith(
            "kotlin-") && (allowAnnotations || jar.name != "annotations.jar") // see comments for [intellijAnnotations] above
    ...
    with(XMLWriter(ivyFile.writer())) {
        document("UTF-8", "1.0") {
            element("ivy-module") {
                ...
                emptyElement("info") {
                    attributes("organisation" to organization, "module" to moduleName, "revision" to version,
                ...
                element("publications") {
                    artifactDir.listFiles()?.filter(::shouldIncludeIntellijJar)?.sortedBy { it.name.lowercase() }
                        ?.forEach { jarFile ->
                            val relativeName = jarFile.toRelativeString(baseDir).removeSuffix(".jar")
                            emptyElement("artifact") {
                                attributes("name" to relativeName, "type" to "jar", "ext" to "jar", "conf" to "default")
                            }
                        }
                    ...
                    sourcesJar.forEach { jarFile ->
                        emptyElement("artifact") {
                            ...
                            attributes("name" to sourcesArtifactName, "type" to "jar", "ext" to "jar",
                                    "conf" to "sources", "m:classifier" to "sources")
                        }
                    }
                }
            }
