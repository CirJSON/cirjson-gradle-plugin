...
abstract class PropertiesValueSource :
        ValueSource<Properties, PropertiesValueSource.Parameters> {
...
fun getRootSettings(settings: Settings,
        gradle: Gradle): Settings { // Gradle interface neither exposes flag if it is a root of composite-build hierarchy
    ...
    return when {
        gradleInternal.isRootBuild() || setOf("gradle-settings-conventions", "gradle-build-conventions").contains(
...
fun loadLocalOrGradleProperty(
        propertyName: String): Provider<String> { // Workaround for https://github.com/gradle/gradle/issues/19114
    ...
    return if ((gradle as GradleInternal).isRootBuild()) {
        localProperties.map { it.getProperty(propertyName) }.orElse(providers.gradleProperty(propertyName))
            .orElse(rootGradleProperties.map { it.getProperty(propertyName) })
    } else {
        localProperties.map { it.getProperty(propertyName) }.orElse(rootSettings.providers.gradleProperty(propertyName))
            .orElse(rootGradleProperties.map { it.getProperty(propertyName) })
    }
}
...
fun RepositoryHandler.addBootstrapRepo(bootstrapRepo: String, bootstrapVersion: String,
        additionalBootstrapRepos: List<String> = emptyList()) {
    exclusiveContent {
        forRepositories(*(listOf(bootstrapRepo) + additionalBootstrapRepos).map {
                maven { url = uri(it) }
            }.toTypedArray())
        filter { // kotlin-build-gradle-plugin and non bootstrap-versions
            // should be excluded from strict content filtering
            includeVersionByRegex("org\\.jetbrains\\.kotlin", "^(?!kotlin-build-gradle-plugin).*$",
                    bootstrapVersion.toRegex().pattern)

            // Kotlin Gradle plugins that have slightly separate maven coordinates
            includeVersionByRegex("org\\.jetbrains\\.kotlin\\..*$", "org\\.jetbrains\\.kotlin\\..*\\.gradle\\.plugin$",
                    bootstrapVersion.toRegex().pattern)
        }
    }
}
...
fun getAdditionalBootstrapRepos(bootstrapRepo: String, bootstrapKotlinVersion: String,
        isJpsBuildEnabled: Boolean): List<String> {
    return buildList {
        if (bootstrapRepo.startsWith("https://buildserver.labs.intellij.net") || bootstrapRepo.startsWith(
        ...
        if (isJpsBuildEnabled) {
            add("https://teamcity.jetbrains.com/guestAuth/app/rest/builds/buildType:(id:Kotlin_KotlinPublic_Aggregate)," + "number:$bootstrapKotlinVersion,branch:default:any/artifacts/content/internal/repo/")
        }
    }
}

fun Settings.applyBootstrapConfiguration(bootstrapVersion: String, bootstrapRepo: String, isJpsBuildEnabled: Boolean,
        logMessage: String) {
...
val isLocalBootstrapEnabled: Provider<Boolean> =
        loadLocalOrGradleProperty(Config.LOCAL_BOOTSTRAP).mapToBoolean().orElse(false)

val localBootstrapVersion: Provider<String> = loadLocalOrGradleProperty(Config.LOCAL_BOOTSTRAP_VERSION).orElse(
            loadLocalOrGradleProperty(Config.DEFAULT_SNAPSHOT_VERSION))
...
val isJpsBuildEnabled = loadLocalOrGradleProperty(Config.IS_JPS_BUILD_ENABLED).mapToBoolean().orElse(false)
...
when {
    isLocalBootstrapEnabled.get() -> {
        ...
        applyBootstrapConfiguration(bootstrapVersion, bootstrapRepo, isJpsBuildEnabled.get(),
    ...
    teamCityBootstrapVersion.orNull != null -> {
        ...
        val bootstrapRepo =
                "$baseRepoUrl/guestAuth/app/rest/builds/buildType:(id:$teamCityProjectId),number:$teamCityBuildNumber,$query/artifacts/content/maven/"
