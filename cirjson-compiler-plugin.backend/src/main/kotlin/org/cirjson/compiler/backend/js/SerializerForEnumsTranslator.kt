...
class SerializerForEnumsTranslator(descriptor: ClassDescriptor, translator: DeclarationBodyVisitor,
        context: TranslationContext) : SerializerJsTranslator(descriptor, translator, context, null) {
    ...
    override fun generateSave(function: FunctionDescriptor) = generateFunction(function) { jsFun, ctx ->
        ...
        val ordinalProp =
            serializableDescriptor.unsubstitutedMemberScope.getContributedVariables(Name.identifier("ordinal"),
        ...
        val call =
            JsInvocation(JsNameRef(encodeEnumF, JsNameRef(jsFun.parameters[0].name)), serialClassDescRef, ordinalRef)
        +call.makeStmt()
    }
    ...
    override fun generateLoad(function: FunctionDescriptor) = generateFunction(function) { jsFun, ctx ->
        ...
        val valuesFunc =
            DescriptorUtils.getFunctionByName(serializableDescriptor.staticScope, StandardNames.ENUM_VALUES)
        val decodeEnumCall =
            JsInvocation(JsNameRef(decodeEnumF, JsNameRef(jsFun.parameters[0].name)), serialClassDescRef)
        val resultCall =
            JsArrayAccess(JsInvocation(ctx.getInnerNameForDescriptor(valuesFunc).makeRef()), decodeEnumCall)
        +JsReturn(resultCall)
    }

    override fun instantiateNewDescriptor(context: TranslationContext, correctThis: JsExpression,
            baseSerialDescImplClass: ClassDescriptor): JsExpression {
        val serialDescForEnums =
            serializerDescriptor.getClassFromInternalSerializationPackage(SerialEntityNames.SERIAL_DESCRIPTOR_FOR_ENUM)
        val ctor = serialDescForEnums.unsubstitutedPrimaryConstructor!!
        return JsNew(context.getInnerReference(ctor),
                listOf(JsStringLiteral(serialName), JsIntLiteral(serializableDescriptor.enumEntries().size)))
    }

    override fun addElementsContentToDescriptor(context: TranslationContext, serialDescriptorInThis: JsNameRef,
