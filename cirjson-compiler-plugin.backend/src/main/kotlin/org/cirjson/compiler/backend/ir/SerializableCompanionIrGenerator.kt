...
class SerializableCompanionIrGenerator(val irClass: IrClass, val serializableIrClass: IrClass,
        compilerContext: SerializationPluginContext) : BaseIrGenerator(irClass, compilerContext) {

    companion object {

        fun getSerializerGetterFunction(serializableIrClass: IrClass): IrSimpleFunction? {
            val irClass =
                    if (serializableIrClass.isSerializableObject) serializableIrClass else serializableIrClass.companionObject()
                            ?: return null
            return irClass.findDeclaration<IrSimpleFunction> {
                it.name == SerialEntityNames.SERIALIZER_PROVIDER_NAME && it.valueParameters.size == serializableIrClass.typeParameters.size && it.valueParameters.all { p -> p.type.isKSerializer() } && it.returnType.isKSerializer()
            }
        }
        ...
        fun generate(irClass: IrClass, context: SerializationPluginContext) {
            ...
            if (serializableClass.shouldHaveGeneratedMethodsInCompanion) {
                SerializableCompanionIrGenerator(irClass, getSerializableClassByCompanion(irClass)!!,
                        context).generate()
    ...
    fun generate() {
        ...
        if (serializableIrClass.isSerializableObject || serializableIrClass.isAbstractOrSealedSerializableClass || serializableIrClass.isSerializableEnum()) {
    ...
    private fun IrBuilderWithScope.patchSerializableClassWithMarkerAnnotation(serializer: IrClass) {
        ...
        val annotationMarkerClass = compilerContext.referenceClass(ClassId(SerializationPackages.packageFqName,
                Name.identifier(SerialEntityNames.ANNOTATION_MARKER_CLASS))) ?: return
        ...
        val annotationCtorCall =
                IrConstructorCallImpl.fromSymbolOwner(startOffset, endOffset, annotationType, annotationCtor).apply {
                    putValueArgument(0, createClassReference(serializer.defaultType, startOffset, endOffset))
                }
    ...
    fun generateLazySerializerGetter(methodDescriptor: IrSimpleFunction) {
        ...
        val kSerializerIrClass = compilerContext.referenceClass(
                ClassId(SerializationPackages.packageFqName, SerialEntityNames.KSERIALIZER_NAME))!!.owner
        val targetIrType = kSerializerIrClass.defaultType.substitute(
                mapOf(kSerializerIrClass.typeParameters[0].symbol to compilerContext.irBuiltIns.anyType))

        val property = addLazyValProperty(irClass, targetIrType, SerialEntityNames.CACHED_SERIALIZER_PROPERTY_NAME) {
            val expr = requireNotNull(serializerInstance(serializer, compilerContext, serializableIrClass.defaultType))
    ...
    fun generateSerializerGetter(methodDescriptor: IrSimpleFunction) {
        addFunctionBody(methodDescriptor) { getter ->
            ...
            val expr = serializerInstance(serializer, compilerContext,
                    serializableIrClass.defaultType) { it, _ -> args[it] }
    ...
    private fun getOrCreateSerializerVarargFactory(): IrSimpleFunction {
        irClass.findDeclaration<IrSimpleFunction> {
            it.name == SerialEntityNames.SERIALIZER_PROVIDER_NAME && it.valueParameters.size == 1 && it.valueParameters.first().isVararg && it.returnType.isKSerializer() && it.isFromPlugin(
                    compilerContext.afterK2)
        }?.let { return it }
        val kSerializerStarType =
                compilerContext.getClassFromRuntime(SerialEntityNames.KSERIALIZER_CLASS).starProjectedType
        val f = irClass.addFunction(SerialEntityNames.SERIALIZER_PROVIDER_NAME.asString(), kSerializerStarType,
    ...
    private fun generateSerializerFactoryIfNeeded(getterDescriptor: IrSimpleFunction) {
        ...
        addFunctionBody(serialFactoryDescriptor) { factory ->
            ...
            val call = irInvoke(IrGetValueImpl(startOffset, endOffset, factory.dispatchReceiverParameter!!.symbol),
                    serializerCall, List(argsSize) { compilerContext.irBuiltIns.anyNType }, serializers,
