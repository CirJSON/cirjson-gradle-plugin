...
import org.cirjson.compiler.diagnostic.serializableAnnotationIsUseless
import org.cirjson.compiler.resolve.*
import org.cirjson.compiler.resolve.SerialEntityNames.ARRAY_MASK_FIELD_MISSING_FUNC_NAME
import org.cirjson.compiler.resolve.SerialEntityNames.CACHED_DESCRIPTOR_FIELD
import org.cirjson.compiler.resolve.SerialEntityNames.SINGLE_MASK_FIELD_MISSING_FUNC_NAME
...
class SerializableCodegenImpl(private val classCodegen: ImplementationBodyCodegen) :
    ...
    private fun initializersMapper(prop: SerializableProperty): Pair<KtExpression, Type> {
        val maybeInit = getProp(prop)?.let {
            it.delegateExpressionOrInitializer ?: throw AssertionError("${it.name} property must have initializer")
        }
    ...
    private fun InstructionAdapter.doGenerateWriteSelf(exprCodegen: ExpressionCodegen) {
        ...
        if (superClass.isInternalSerializable) {
            ...
            superTypeArguments.forEach {
                val genericIdx =
                    serializableDescriptor.defaultType.arguments.indexOf(it).let { if (it == -1) null else it }
                val serial = findTypeSerializerOrContext(serializableDescriptor.module, it.type)
                stackValueSerializerInstance(exprCodegen, classCodegen, serializableDescriptor.module, it.type, serial,
                        this, genericIdx) { i, _ ->
                    load(offsetI + i, kSerializerType)
                }
            }
            val superSignature = classCodegen.typeMapper.mapSignatureSkipGeneric(
                    KSerializerDescriptorResolver.createWriteSelfFunctionDescriptor(superClass))
            invokestatic(classCodegen.typeMapper.mapType(superClass).internalName, superSignature.asmMethod.name,
                    superSignature.asmMethod.descriptor, false)
        } else {
            myPropsStart = 0
        }
        ...
        for (i in myPropsStart until properties.serializableProperties.size) {
            ...
            if (!property.optional) {
                ...
                val actualType: JvmKotlinType =
                    ImplementationBodyCodegen.genPropertyOnStack(this, exprCodegen.context, property.descriptor,
                            thisAsmType, thisI, classCodegen.state)
                ...
                DescriptorAsmUtil.genEqualsForExpressionsOnStack(KtTokens.EXCLEQ, lhs, rhs)
                        .put(Type.BOOLEAN_TYPE, null, this)
                ifne(writeLabel)
                ...
                load(serialDescI, descType)
                iconst(i)
                invokeinterface(kOutputType.internalName, CallingConventions.shouldEncodeDefault,
                        "(${descType.descriptor}I)Z")
                ifeq(nonWriteLabel)
    ...
    private fun InstructionAdapter.doGenerateConstructorImpl(exprCodegen: ExpressionCodegen) {
        ...
        for (i in propIndex until properties.serializableProperties.size) {
            ...
            if (!prop.optional) {
                ...
                if (classCodegen.bindingContext[BindingContext.BACKING_FIELD_REQUIRED, prop.descriptor] != true) throw CompilationException(
        ...
        (descToProps - serializedProps).filter { classCodegen.shouldInitializeProperty(it.value) }
                .forEach { (_, prop) -> classCodegen.initializeProperty(exprCodegen, prop) }
        (paramsToProps - serializedProps).forEach { (t, u) -> exprCodegen.genInitParam(t, u) }
        ...
        for (specifier in classCodegen.myClass.superTypeListEntries) {
            if (specifier is KtDelegatedSuperTypeEntry) {
                ...
                putfield(thisAsmType.internalName, "\$\$delegate_${delegate++}", stackValue.type.descriptor)
            }
        }

        // init blocks
        // todo: proper order with other initializers?
        classCodegen.myClass.anonymousInitializers().forEach { exprCodegen.gen(it, Type.VOID_TYPE) }
        areturn(Type.VOID_TYPE)
    }
    ...
    private fun InstructionAdapter.generateOptimizedGoldenMaskCheck(maskVar: Int) {
        ...
        if (maskSlotCount == 1) {
            ...
            invokestatic(pluginUtilsType.internalName, SINGLE_MASK_FIELD_MISSING_FUNC_NAME.asString(),
                    "(II${descType.descriptor})V", false)
        } else {
            ...
            stackSerialDescriptor()
            invokestatic(pluginUtilsType.internalName, ARRAY_MASK_FIELD_MISSING_FUNC_NAME.asString(),
                    "([I[I${descType.descriptor})V", false)
        }
    ...
    private fun generateStaticDescriptorField() {
        ...
        classCodegen.v.newField(OtherOrigin(classCodegen.myClass.psiOrParent), flags, CACHED_DESCRIPTOR_FIELD,
                descType.descriptor, null, null)
    ...
    private fun ExpressionCodegen.genInitProperty(prop: SerializableProperty) = getProp(prop)?.let {
        ...
        if (!it.hasDefaultValue()) throw CompilationException(
                "Optional field ${it.name} in primary constructor of serializable " + "$serializableDescriptor must have default value",
                null, it)
    ...
    private fun ExpressionCodegen.genInitParam(prop: PropertyDescriptor, param: KtParameter) {
        ...
        if (!param.hasDefaultValue()) throw CompilationException(
                "Transient field ${param.name} in primary constructor of serializable " + "$serializableDescriptor must have default value",
                null, param)
    ...
    private fun canUseFieldMissingOptimization(): Boolean {
        val implementationVersion = VersionReader.getVersionsForCurrentModuleFromContext(currentDeclaration.module,
                bindingContext)?.implementationVersion
        ...
        fun generateSerializableExtensions(codegen: ImplementationBodyCodegen) {
            ...
            if (serializableClass.isInternalSerializable) {
                ...
                throw CompilationException(
                        "@Serializable annotation on $serializableClass would be ignored because it is impossible to serialize it automatically. " + "Provide serializer manually via e.g. companion object",
