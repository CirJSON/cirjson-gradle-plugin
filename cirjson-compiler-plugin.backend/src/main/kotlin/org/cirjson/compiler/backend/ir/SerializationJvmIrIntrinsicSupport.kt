...
class SerializationJvmIrIntrinsicSupport(private val jvmBackendContext: JvmBackendContext,
        private val irPluginContext: IrPluginContext) : SerializationBaseContext, JvmIrIntrinsicExtension {

    sealed class IntrinsicType(val methodDescriptor: String) {

        object Simple : IntrinsicType(stubCallDescriptor)

        class WithModule(val storedIndex: Int) : IntrinsicType(stubCallDescriptorWithModule)
    ...
    inner class ReifiedSerializerMethod(private val withModule: Boolean) : IntrinsicMethod() {
        ...
        override fun invoke(expression: IrFunctionAccessExpression, codegen: ExpressionCodegen,
                data: BlockInfo): PromisedValue {
            with(codegen) {
                ...
                generateSerializerForType(argument, mv, intrinsicType)
                codegen.propagateChildReifiedTypeParametersUsages(
                        codegen.typeMapper.typeSystem.extractUsedReifiedParameters(argument))
                if (withModule) {
    ...
    companion object {
        ...
        val stubCallDescriptorWithModule =
                "(${serializersModuleType.descriptor}${kTypeType.descriptor})${kSerializerType.descriptor}"
    ...
    override fun getIntrinsic(symbol: IrFunctionSymbol): IntrinsicMethod? {
        ...
        if (!method.isTargetMethod() || method.dispatchReceiverParameter != null || method.typeParameters.size != 1 || method.valueParameters.isNotEmpty()) return null
        val receiver = method.extensionReceiverParameter
        return if (receiver == null) ReifiedSerializerMethod(withModule = false)
        else if (receiver.type.classFqName?.asString() == "kotlinx.serialization.modules.SerializersModule") ReifiedSerializerMethod(
                withModule = true)
        else null
    }
    ...
    private val currentVersion by lazy {
        VersionReader.getVersionsForCurrentModuleFromTrace(module,
                jvmBackendContext.state.bindingTrace)?.implementationVersion
    }
     ...
     */
    override fun rewritePluginDefinedOperationMarker(v: InstructionAdapter, reifiedInsn: AbstractInsnNode,
            instructions: InsnList, type: IrType): Boolean {
        ...
        } else IntrinsicType.Simple // Remove other instructions
        ...
        instructions.remove(reifiedInsn) // generate serializer
    ...
    private fun InstructionAdapter.putReifyMarkerIfNeeded(type: IrType, intrinsicType: IntrinsicType): Boolean =
            with(typeSystemContext) {
                ...
                if (typeDescriptor != null) { // need further reification
                    ReifiedTypeInliner.putReifiedOperationMarkerIfNeeded(typeDescriptor, type.isMarkedNullable(),
                            ReifiedTypeInliner.OperationKind.TYPE_OF, this@putReifyMarkerIfNeeded, typeSystemContext)
                    aconst(null)
                    aconst(intrinsicType.magicMarkerString())
                    invokestatic(ReifiedTypeInliner.pluginIntrinsicsMarkerOwner,
                            ReifiedTypeInliner.pluginIntrinsicsMarkerMethod,
                            ReifiedTypeInliner.pluginIntrinsicsMarkerSignature, false)
                    if (intrinsicType is IntrinsicType.WithModule) { // Force emit load instruction so we can retrieve var index later
    ...
    fun generateSerializerForType(type: IrType, adapter: InstructionAdapter, intrinsicType: IntrinsicType) {
        ...
        if (serializerMethod != null) { // fast path
            val companionType =
                    if (typeDescriptor.isSerializableObject) typeDescriptor else typeDescriptor.companionObject()!!
            ...
            adapter.invokevirtual(typeMapper.mapClass(companionType).internalName, "serializer",
                    "(${signature})${kSerializerType.descriptor}", false)
        } else { // More general path, including special or built-in serializers for e.g. List
            val serializer = support.findTypeSerializerOrContext(type)
            support.stackValueSerializerInstance(type, serializer, adapter, intrinsicType) { genericArg ->
    ...
    private fun stackValueSerializerInstance(kType: IrType, maybeSerializer: IrClassSymbol?, iv: InstructionAdapter,
            intrinsicType: IntrinsicType, genericIndex: Int? = null,
            genericSerializerFieldGetter: (InstructionAdapter.(IrType) -> Unit)? = null): Boolean {
        if (maybeSerializer == null && genericIndex != null) { // get field from serializer object
        ...
        if (maybeSerializer == null && intrinsicType == IntrinsicType.Simple) {
            iv.apply {
                ...
                invokestatic(serializersKtInternalName, noCompiledSerializerMethodName,
                        "(Ljava/lang/String;)${kSerializerType.descriptor}", false)
            }
            return false
        }
        if (maybeSerializer != null && maybeSerializer.owner.isObject) { // singleton serializer -- just get it
        ...
        val argSerializers = typeArgumentsAsTypes.map { argType -> // check if any type argument is not serializable
            val argSerializer = if (argType.isTypeParameter()) null else findTypeSerializerOrContext(
                    argType) // check if it can be properly serialized with its args recursively
        ...
        fun instantiate(typeArgument: Pair<IrType, IrClassSymbol?>, signature: StringBuilder?) {
            ...
            stackValueSerializerInstance(argType, argSerializer, iv, intrinsicType, argType.genericIndex,
                    genericSerializerFieldGetter) // wrap into nullable serializer if argType is nullable
        ...
        val serializer = maybeSerializer ?: iv.run { // insert noCompilerSerializer(module, kClass, arguments)
            ...
            // SerializersModule
            load(intrinsicType.storedIndex, serializersModuleType) // KClass
            aconst(typeMapper.mapTypeCommon(kType, TypeMappingMode.GENERIC_ARGUMENT))
            AsmUtil.wrapJavaClassIntoKClass(this)

            val descriptor = StringBuilder(
                    "(${serializersModuleType.descriptor}${AsmTypes.K_CLASS_TYPE.descriptor}") // Generic args (if present)
            ...
            invokestatic(serializersKtInternalName, noCompiledSerializerMethodName, descriptor.toString(), false)
            return false
        }
        ...
        iv.apply {
            ...
            if (serializer.owner.classId == enumSerializerId && runtimeHasEnumSerializerFactoryFunctions) {
                val enumIrClass = kType.classOrNull!!.owner // runtime contains enum serializer factory functions
            ...
            anew(serializerType)
            dup() // instantiate all arg serializers on stack
            val signature = StringBuilder("(")
            val serialName = kType.serialName()
            when (serializer.owner.classId) {
                enumSerializerId -> { // support legacy serializer instantiation by constructor for old runtimes
                    aconst(serialName)
                ...
                contextSerializerId, polymorphicSerializerId -> { // a special way to instantiate enum -- need a enum KClass reference
                    // GENERIC_ARGUMENT forces boxing in order to obtain KClass
                    aconst(typeMapper.mapTypeCommon(kType, TypeMappingMode.GENERIC_ARGUMENT))
                    AsmUtil.wrapJavaClassIntoKClass(this)
                    signature.append(AsmTypes.K_CLASS_TYPE.descriptor)
                    if (serializer.owner.classId == contextSerializerId && hasNewContextSerializerSignature) { // append new additional arguments
                        val fallbackDefaultSerializer =
                                findTypeSerializer(this@SerializationJvmIrIntrinsicSupport, kType)
                ...
                referenceArraySerializerId -> { // a special way to instantiate reference array serializer -- need an element KClass reference
                    aconst(typeMapper.mapTypeCommon(typeArgumentsAsTypes.first(), TypeMappingMode.GENERIC_ARGUMENT))
                    AsmUtil.wrapJavaClassIntoKClass(this)
                    signature.append(
                            AsmTypes.K_CLASS_TYPE.descriptor) // Reference array serializer still needs serializer for its argument type
                ...
                sealedSerializerId -> {
                    ...
                    val (subClasses, subSerializers) = emptyGenerator.allSealedSerializableSubclassesFor(
                            ...
                            this@SerializationJvmIrIntrinsicSupport) // KClasses vararg
                    fillArray(AsmTypes.K_CLASS_TYPE, subClasses) { _, type ->
                        aconst(typeMapper.mapTypeCommon(type, TypeMappingMode.GENERIC_ARGUMENT))
                        AsmUtil.wrapJavaClassIntoKClass(this)
                    }
                    signature.append(AsmTypes.K_CLASS_ARRAY_TYPE.descriptor) // Serializers vararg
                    fillArray(kSerializerType, subSerializers) { i, serializer ->
                        val (argType, argSerializer) = subClasses[i] to serializer
                        assert(stackValueSerializerInstance(argType, argSerializer, this, intrinsicType,
                                argType.genericIndex) { genericType -> // if we encountered generic type parameter in one of subclasses of sealed class, use polymorphism from upper bound
                            assert(stackValueSerializerInstance(
                                    (genericType.classifierOrNull as IrTypeParameterSymbol).owner.representativeUpperBound,
                                    jvmBackendContext.referenceClass(module.getClassFromSerializationPackage(
                                            SpecialBuiltins.polymorphicSerializer)), this, intrinsicType))
                        })
            ...
            signature.append(")V") // invoke constructor
