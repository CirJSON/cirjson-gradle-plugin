 ...
 */
class IrPreGenerator(val irClass: IrClass, compilerContext: SerializationPluginContext) :
    ...
    private fun preGenerateWriteSelfMethodIfNeeded() { // write$Self in K1 is created only on JVM (see SerializationResolveExtension)
        ...
        if (!irClass.shouldHaveSpecificSyntheticMethods {
                    serializerDescriptor.findPluginGeneratedMethod(SerialEntityNames.SAVE, compilerContext.afterK2)
                }) return
        ...
        val typeParams = irClass.typeParameters.map {
            method.addTypeParameter(it.name.asString(), compilerContext.irBuiltIns.anyNType)
        }
        val typeParamsAsArguments = typeParams.map { it.defaultType }

        // object
        method.addValueParameter(Name.identifier("self"), irClass.typeWith(typeParamsAsArguments),
                SERIALIZATION_PLUGIN_ORIGIN) // encoder
        method.addValueParameter(Name.identifier("output"),
                compilerContext.getClassFromRuntime(SerialEntityNames.STRUCTURE_ENCODER_CLASS).defaultType,
                SERIALIZATION_PLUGIN_ORIGIN) // descriptor
        val serialDescriptorSymbol = compilerContext.getClassFromRuntime(SerialEntityNames.SERIAL_DESCRIPTOR_CLASS)
        method.addValueParameter(Name.identifier("serialDesc"), serialDescriptorSymbol.defaultType,
                SERIALIZATION_PLUGIN_ORIGIN) // KSerializer<Tn>
        ...
        typeParamsAsArguments.forEachIndexed { i, it ->
            method.addValueParameter(Name.identifier("${SerialEntityNames.typeArgPrefix}$i"),
                    kSerializerSymbol.typeWith(it), SERIALIZATION_PLUGIN_ORIGIN)
        }
    }

    private fun preGenerateDeserializationConstructorIfNeeded() {
        if (!irClass.isInternalSerializable) return // do not add synthetic deserialization constructor if .deserialize method is customized
        if (irClass.hasCompanionObjectAsSerializer && irClass.companionObject()
                    ?.findPluginGeneratedMethod(SerialEntityNames.LOAD, compilerContext.afterK2) == null) return
        ...
        val markerClassSymbol = compilerContext.getClassFromInternalSerializationPackage(
                SerialEntityNames.SERIAL_CTOR_MARKER_NAME.asString())
        ...
        repeat(bitMaskSlotsCount) {
            ctor.addValueParameter(Name.identifier("seen$it"), compilerContext.irBuiltIns.intType,
                    SERIALIZATION_PLUGIN_ORIGIN)
        }

        for (prop in serializableProperties) { // SerialName can contain illegal identifier characters, so we use original source code name for parameter
            ctor.addValueParameter(prop.originalDescriptorName, prop.type.makeNullableIfNotPrimitive(),
                    SERIALIZATION_PLUGIN_ORIGIN)
        }

        ctor.addValueParameter(SerialEntityNames.dummyParamName, markerClassSymbol.defaultType.makeNullable(),
                SERIALIZATION_PLUGIN_ORIGIN)
    }

    private fun IrType.makeNullableIfNotPrimitive() = if (this.isPrimitiveType(false)) {
        this
    } else {
        this.makeNullable()
    }
    ...
    companion object {
        ...
        fun generate(irClass: IrClass, compilerContext: SerializationPluginContext) {
            if (!irClass.isInternalSerializable) return
