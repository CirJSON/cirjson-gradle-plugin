...
import org.cirjson.compiler.backend.common.SerializableCodegen
import org.cirjson.compiler.backend.common.anonymousInitializers
...
class SerializableJsTranslator(val declaration: KtPureClassOrObject, val descriptor: ClassDescriptor,
        val context: TranslationContext) : SerializableCodegen(descriptor, context.bindingContext()) {

    private val initMap: Map<PropertyDescriptor, KtExpression?> =
        context.buildInitializersRemapping(declaration, descriptor.getSuperClassNotAny())

    override fun generateInternalConstructor(constructorDescriptor: ClassConstructorDescriptor) {

        val missingExceptionClassRef = serializableDescriptor.getClassFromSerializationPackage(
                MISSING_FIELD_EXC).constructors.single { it.valueParameters.size == 1 }
        ...
        val f = context.buildFunction(constructorDescriptor) { jsFun, context ->
            ...
            @Suppress("NAME_SHADOWING") val context =
                context.innerContextWithAliased(serializableDescriptor.thisAsReceiverParameter, thiz)
            ...
            +JsVars(JsVars.JsVar(thiz.name, JsAstUtils.or(markerAsThis, Namer.createObjectWithPrototypeFrom(
                    context.getInnerNameForDescriptor(serializableDescriptor).makeRef()))))
            ...
            when {
                ...
                superClass.isInternalSerializable -> {
                    startPropOffset =
                        generateSuperSerializableCall(superClass, jsFun.parameters.map { it.name.makeRef() }, thiz,
                                seenVarsOffset)
                }
            ...
            for (index in startPropOffset until serializableProperties.size) {
                ...
                val assignParamStmt =
                    TranslationUtils.assignmentToBackingField(context, prop.descriptor, paramRef).makeStmt()

                val ifNotSeenStmt: JsStatement = if (prop.optional) {
                    val initializer = initMap.getValue(prop.descriptor) ?: throw IllegalArgumentException(
                            "optional without an initializer")
                    ...
                    JsThrow(if (missingExceptionClassRef.isPrimary) {
                        JsNew(context.translateQualifiedReference(missingExceptionClassRef.containingDeclaration),
                                listOf(JsStringLiteral(prop.name)))
                    } else {
                        JsInvocation(context.getInnerNameForDescriptor(missingExceptionClassRef).makeRef(),
                                listOf(JsStringLiteral(prop.name)))
                    })
                }
                // (seen & 1 << i == 0) -- not seen
            ...
            declaration.anonymousInitializers().forEach { Translation.translateAsExpression(it, context, this.block) }
    ...
    private fun JsBlockBuilder.generateSuperNonSerializableCall(superClass: ClassDescriptor,
            thisParameter: JsExpression) {
        val suitableCtor =
            superClass.constructors.singleOrNull { it.valueParameters.size == 0 } ?: throw IllegalArgumentException(
        ...
        if (suitableCtor.isPrimary) {
            ...
            +JsAstUtils.assignment(thisParameter, JsInvocation(context.getInnerReference(suitableCtor), thisParameter))
                    .makeStmt()
        }
    }

    private fun JsBlockBuilder.generateSuperSerializableCall(superClass: ClassDescriptor,
            parameters: List<JsExpression>, thisParameter: JsExpression, propertiesStart: Int): Int {
        ...
        val arguments = parameters.subList(0, superSlots) + parameters.subList(propertiesStart,
                propertiesStart + superProperties.size) + thisParameter // SerializationConstructorMarker
    ...
    companion object {
        ...
        fun translate(declaration: KtPureClassOrObject, serializableClass: ClassDescriptor,
                context: TranslationContext) {
            if (serializableClass.isInternalSerializable) SerializableJsTranslator(declaration, serializableClass,
                    context).generate()
