...
import org.cirjson.compiler.resolve.SerialEntityNames.CACHED_CHILD_SERIALIZERS_PROPERTY_NAME
...
import org.cirjson.compiler.resolve.SerialEntityNames.LOAD
import org.cirjson.compiler.resolve.SerialEntityNames.SAVE
...
import org.cirjson.compiler.resolve.bitMaskSlotCount
...
open class SerializerIrGenerator(val irClass: IrClass, compilerContext: SerializationPluginContext,
        metadataPlugin: SerializationDescriptorSerializerPlugin?) : BaseIrGenerator(irClass, compilerContext) {
    ...
    protected val generatedSerialDescPropertyDescriptor =
            getProperty(SerialEntityNames.SERIAL_DESC_FIELD) { true }?.takeIf {
                it.isFromPlugin(compilerContext.afterK2)
            }

    protected val irAnySerialDescProperty = getProperty(SerialEntityNames.SERIAL_DESC_FIELD) { true }

    fun getProperty(name: String, isReturnTypeOk: (IrProperty) -> Boolean): IrProperty? {
    ...
    private val cachedChildSerializersProperty by lazy {
        if (isGeneratedSerializer) serializableIrClass.companionObject()?.properties?.singleOrNull { it.name == CACHED_CHILD_SERIALIZERS_PROPERTY_NAME }
        else null
    }

    // non-object serializers which can be cached
    private val cacheableChildSerializers by lazy {
        serializableIrClass.createCachedChildSerializers(serializableIrClass, properties.serializableProperties)
            .map { it != null }
    }
    ...
    fun generateSerialDesc() {
        ...
        compilerContext.symbolTable.withReferenceScope(irClass) {
            prop = generatePropertyMissingParts(desc, desc.name, serialDescImplClass.starProjectedType, irClass,
                    desc.visibility)

            localSerializersFieldsDescriptors = findLocalSerializersFieldDescriptors().mapIndexed { i, prop ->
                generatePropertyMissingParts(prop, Name.identifier("${SerialEntityNames.typeArgPrefix}$i"),
                        compilerContext.getClassFromRuntime(KSERIALIZER_CLASS).starProjectedType, irClass)
            }
        }

        irClass.addAnonymousInit {
            val localDesc = irTemporary(instantiateNewDescriptor(serialDescImplClass, irGet(thisAsReceiverParameter)),
                    nameHint = "serialDesc")

            addElementsContentToDescriptor(serialDescImplClass, localDesc, addFuncS) // add class annotations
            copySerialInfoAnnotationsToDescriptor(collectSerialInfoAnnotations(serializableIrClass), localDesc,
                    serialDescImplClass.functionByName(CallingConventions.addClassAnnotation))

            // save local descriptor to field
            +irSetField(IrGetValueImpl(startOffset, endOffset, thisAsReceiverParameter.symbol), prop.backingField!!,
                    irGet(localDesc))
        }
    }

    protected open fun IrBlockBodyBuilder.instantiateNewDescriptor(serialDescImplClass: IrClassSymbol,
            correctThis: IrExpression): IrExpression { //        val classConstructors = compilerContext.referenceConstructors(serialDescImplClass.fqNameSafe)
        val serialClassDescImplCtor = serialDescImplClass.constructors.single { it.owner.isPrimary }
        return irInvoke(null, serialClassDescImplCtor, irString(serialName),
                if (isGeneratedSerializer) correctThis else irNull(), irInt(serializableProperties.size))
    }

    protected open fun IrBlockBodyBuilder.addElementsContentToDescriptor(serialDescImplClass: IrClassSymbol,
            localDescriptor: IrVariable, addFunction: IrFunctionSymbol) {
        fun addFieldCall(prop: IrSerializableProperty) =
                irInvoke(irGet(localDescriptor), addFunction, irString(prop.name), irBoolean(prop.optional),
                        typeHint = compilerContext.irBuiltIns.unitType)

        for (classProp in serializableProperties) {
            if (classProp.transient) continue
            +addFieldCall(classProp) // add property annotations
            val property = classProp.ir //.getIrPropertyFrom(serializableIrClass)
            copySerialInfoAnnotationsToDescriptor(property.annotations, localDescriptor,
                    serialDescImplClass.functionByName(CallingConventions.addAnnotation))
        }
    }

    protected fun IrBlockBodyBuilder.copySerialInfoAnnotationsToDescriptor(annotations: List<IrConstructorCall>,
            receiver: IrVariable, method: IrFunctionSymbol) {
        copyAnnotationsFrom(annotations).forEach {
            +irInvoke(irGet(receiver), method, it)
        }
    }

    fun generateGenericFieldsAndConstructor(typedConstructorDescriptor: IrConstructor) = addFunctionBody(
            typedConstructorDescriptor) { ctor -> // generate call to primary ctor to init serialClassDesc and super()
        val primaryCtor = irClass.primaryConstructorOrFail
        +IrDelegatingConstructorCallImpl.fromSymbolOwner(startOffset, endOffset, compilerContext.irBuiltIns.unitType,
                primaryCtor.symbol).apply {
        ...
        ctor.valueParameters.forEachIndexed { index, param ->
            ...
            +irSetField(IrGetValueImpl(startOffset, endOffset, thisAsReceiverParameter.symbol), localSerial,
                    irGet(param))
        }
    }

    open fun generateChildSerializersGetter(function: IrSimpleFunction) = addFunctionBody(function) { irFun ->
        val cachedChildSerializerByIndex = createCacheableChildSerializersFactory(cachedChildSerializersProperty,
                cacheableChildSerializers) { serializableIrClass.companionObject()!! }

        val allSerializers = serializableProperties.mapIndexed { index, property ->
            requireNotNull(serializerTower(this@SerializerIrGenerator, irFun.dispatchReceiverParameter!!, property,
                    cachedChildSerializerByIndex(index))) { "Property ${property.name} must have a serializer" }
        }
    ...
    open fun generateTypeParamsSerializersGetter(function: IrSimpleFunction) = addFunctionBody(function) { irFun ->
        val typeParams = serializableIrClass.typeParameters.mapIndexed { idx, _ ->
            irGetField(irGet(irFun.dispatchReceiverParameter!!), localSerializersFieldsDescriptors[idx].backingField!!)
        }
        val kSerType = ((irFun.returnType as IrSimpleType).arguments.first() as IrTypeProjection).type
    ...
    open fun generateSerializableClassProperty(
            property: IrProperty) {/* Already implemented in .generateSerialClassDesc ? */
    }

    open fun generateSave(function: IrSimpleFunction) = addFunctionBody(function) { saveFunc ->

        fun irThis(): IrExpression = IrGetValueImpl(startOffset, endOffset, saveFunc.dispatchReceiverParameter!!.symbol)
        ...
        val call = irInvoke(irGet(saveFunc.valueParameters[0]), beginFunc, irGet(localSerialDesc),
                typeHint = kOutputClass.defaultType)
        ...
        if (writeSelfFunction != null) { // extract Tx from KSerializer<Tx> list
            val typeArgs =
            ...
            val initializerAdapter: (IrExpressionBody) -> IrExpression =
                    createInitializerAdapter(serializableIrClass, propertyByParamReplacer,
            ...
            serializeAllProperties(serializableProperties, objectToSerialize, localOutput, localSerialDesc,
                    kOutputClass, ignoreIndexTo = -1, initializerAdapter, cachedChildSerializerByIndex) { it, _ ->
    ...
    protected fun IrBlockBodyBuilder.formEncodeDecodePropertyCall(encoder: IrExpression,
            dispatchReceiver: IrValueParameter, property: IrSerializableProperty,
            ...
            returnTypeHint: IrType? = null): IrExpression =
            formEncodeDecodePropertyCall(encoder, property, whenHaveSerializer, whenDoNot, cachedSerializer, { it, _ ->
                val ir = localSerializersFieldsDescriptors[it]
    ...
    private fun IrBuilderWithScope.defaultValueAndType(descriptor: IrProperty): Pair<IrExpression, IrType> {
        ...
        val defaultPrimitive: IrExpression? = if (T.isMarkedNullable()) null
        ...
        return if (defaultPrimitive == null) T.makeNullable().let { irNull(it) to it }
        else defaultPrimitive to T
    }
    ...
    open fun generateLoad(function: IrSimpleFunction) = addFunctionBody(function) { loadFunc ->
        ...
        fun irThis(): IrExpression = IrGetValueImpl(startOffset, endOffset, loadFunc.dispatchReceiverParameter!!.symbol)
        ...
        val serialPropertiesIndexes =
                serializableProperties.mapIndexed { i, property -> property to i }.associate { (p, i) -> p.ir to i }

        val transients = serializableIrClass.declarations.asSequence().filterIsInstance<IrProperty>()
            .filter { !serialPropertiesIndexes.contains(it) }.filter { it.backingField != null }

        // var bitMask0 = 0, bitMask1 = 0...
        val bitMasks = (0 until blocksCnt).map {
            irTemporary(irInt(0), "bitMask$it", isMutable = true)
        } // var local0 = null, local1 = null ...
        val serialPropertiesMap =
                serializableProperties.mapIndexed { i, prop -> i to prop.ir }.associate { (i, descriptor) ->
        ...
        val call = irInvoke(irGet(loadFunc.valueParameters[0]), beginFunc, irGet(localSerialDesc),
        ...
        val decoderCalls: List<Pair<Int, IrExpression>> = serializableProperties.mapIndexed { index, property ->
            val body = irBlock {
                val decodeFuncToCall =
                        formEncodeDecodePropertyCall(localInput.get(), loadFunc.dispatchReceiverParameter!!, property,
                                { innerSerial, sti ->
                                    inputClass.functions.single {
                                        it.owner.name.asString() == "${CallingConventions.decode}${sti.elementMethodPrefix}Serializable${CallingConventions.elementPostfix}" && it.owner.valueParameters.size == 4
                                    } to listOf(localSerialDesc.get(), irInt(index), innerSerial,
                                            serialPropertiesMap.getValue(property.ir).get())
                                }, { sti ->
                            inputClass.functions.single {
                                it.owner.name.asString() == "${CallingConventions.decode}${sti.elementMethodPrefix}${CallingConventions.elementPostfix}" && it.owner.valueParameters.size == 2
                            } to listOf(localSerialDesc.get(), irInt(index))
                        }, cachedChildSerializerByIndex(index),
                                returnTypeHint = property.type) // local$i = localInput.decode...(...)
                +irSet(serialPropertiesMap.getValue(property.ir).symbol, decodeFuncToCall) // bitMask[i] |= 1 << x
        ...
        val decodeSequentiallyCall =
                irInvoke(localInput.get(), inputClass.functionByName(CallingConventions.decodeSequentially))
        ...
        val byIndexPart: IrExpression = irWhile().also { loop ->
            ...
            loop.body = irBlock {
                ...
                +irWhen { // if index == -1 (READ_DONE) break loop
                    ...
                    val excClassRef = compilerContext.referenceConstructors(
                            ClassId(SerializationPackages.packageFqName, Name.identifier(UNKNOWN_FIELD_EXC)))
        ...
        +irInvoke(localInput.get(), endFunc, irGet(localSerialDesc))
        ...
        if (serializableIrClass.isInternalSerializable && deserCtor != null) {
            ...
            if (irClass.isLocal) { // if the serializer is local, then the serializable class too, since they must be in the same scope
            ...
            val ctorArgs = params.map { parameter ->
                ...
                if (serialProperty != null) {
                    ...
                    if (parameter.hasDefaultValue()) {
                        val propNotSeenTest = irEquals(irInt(0),
                                irBinOp(OperatorNameConventions.AND, bitMasks[index / 32].get(),
            ...
            generateSetStandaloneProperties(serializerVar, serialPropertiesMap::getValue,
                    serialPropertiesIndexes::getValue, bitMasks)
            +irReturn(irGet(serializerVar))
        }
    }

    private fun IrBlockBodyBuilder.generateSetStandaloneProperties(serializableVar: IrVariable,
            propVars: (IrProperty) -> IrVariable, propIndexes: (IrProperty) -> Int, bitMasks: List<IrVariable>) {
        for (property in properties.serializableStandaloneProperties) {
            val localPropIndex = propIndexes(property.ir) // generate setter call
            val setter = property.ir.setter!!
            val propSeenTest = irNotEquals(irInt(0),
                    irBinOp(OperatorNameConventions.AND, irGet(bitMasks[localPropIndex / 32]),
                            irInt(1 shl (localPropIndex % 32))))

            val setterInvokeExpr =
                    irSet(setter.returnType, irGet(serializableVar), setter.symbol, irGet(propVars(property.ir)))
    ...
    fun generate() {
        ...
        if (serializableIrClass.typeParameters.isNotEmpty()) {
            findSerializerConstructorForTypeArgumentsSerializers(irClass)?.takeIf { it.owner.isFromPlugin(withFir) }
                ?.let {
    ...
    companion object {
        ...
        fun generate(irClass: IrClass, context: SerializationPluginContext,
            ...
            generator.generate()
            if (irClass.isFromPlugin(context.afterK2)) { // replace origin only for plugin generated serializers
