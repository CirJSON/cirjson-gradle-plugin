...
import org.cirjson.compiler.resolve.SerialEntityNames.CACHED_DESCRIPTOR_FIELD_NAME
...
class SerializableIrGenerator(val irClass: IrClass, compilerContext: SerializationPluginContext) :
        BaseIrGenerator(irClass, compilerContext) {

    protected val properties = serializablePropertiesForIrBackend(irClass)

    private val serialDescriptorClass =
            compilerContext.referenceClass(SerializationRuntimeClassIds.descriptorClassId)!!.owner

    private val serialDescriptorImplClass = compilerContext.referenceClass(
            ClassId(SerializationPackages.internalPackageFqName,
                    Name.identifier(SerialEntityNames.SERIAL_DESCRIPTOR_CLASS_IMPL)))!!.owner
    ...
    fun generateInternalConstructor(constructorDescriptor: IrConstructor) =
            addFunctionBody(constructorDescriptor) { ctor ->
                ...
                val statementsAfterSerializableProperty: MutableMap<IrProperty?, MutableList<IrStatement>> =
                        mutableMapOf()
                irClass.declarations.asSequence().forEach {
                    when { // only properties with backing field
                        it is IrProperty && it.backingField != null -> {
                            if (it in serialDescs) {
                                current = it
                            } else if (it.backingField?.initializer != null && !(it.isJvmOptimizableDelegate() && compilerContext.platform.isJvm())) { // skip transient lateinit or deferred properties (with null initializer) and optimized delegations
                ...
                val exceptionCtorRef = compilerContext.referenceConstructors(
                        ClassId(SerializationPackages.packageFqName, Name.identifier(MISSING_FIELD_EXC)))
                ...
                if (useFieldMissingOptimization() && // for abstract classes fields MUST BE checked in child classes
                    ...
                    } else { // synthetic constructor is created only for internally serializable classes - so companion definitely exists
                        val companionObject = irClass.companionObject()!!
                        getParametrizedSerialDescriptorExpr(companionObject,
                                createCachedDescriptorProperty(companionObject))
                    }
                    generateGoldenMaskCheck(seenVars, properties, getDescriptorExpr)
                }
                when {
                    superClass.symbol == compilerContext.irBuiltIns.anyClass -> generateAnySuperConstructorCall(
                            toBuilder = this@addFunctionBody)
                    superClass.isInternalSerializable -> {
                        startPropOffset =
                                generateSuperSerializableCall(superClass, ctor.valueParameters, seenVarsOffset)
                    }
                ...
                for (index in startPropOffset until serializableProperties.size) {
                    ...
                    val paramRef =
                            ctor.valueParameters[index + seenVarsOffset] // Assign this.a = a in else branch // Set field directly w/o setter to match behavior of old backend plugin
                    ...
                    val ifNotSeenExpr: IrExpression = if (prop.optional) {
                        val initializerBody = requireNotNull(initializerAdapter(
                                prop.ir.backingField?.initializer!!)) { "Optional value without an initializer" } // todo: filter abstract here
                        irSetField(irGet(thiz), backingFieldToAssign, initializerBody)
                    } else { // property required
                        if (useFieldMissingOptimization()) { // field definitely not empty as it's checked before - no need another IF, only assign property from param
                            +assignParamExpr
                    ...
                    val propNotSeenTest = irEquals(irInt(0),
                            irBinOp(OperatorNameConventions.AND, irGet(seenVars[bitMaskSlotAt(index)]),
                ...
                irClass.declarations.filterIsInstance<IrField>().filter { it.origin == IrDeclarationOrigin.DELEGATE }
                    .forEach {
                        val receiver = if (!it.isStatic) irGet(thiz) else null
                        +irSetField(receiver, it, initializerAdapter(it.initializer!!),
                                IrStatementOrigin.INITIALIZE_FIELD)
                    }
            }

    private fun IrBlockBodyBuilder.getStaticSerialDescriptorExpr(): IrExpression {
        val serializerIrClass = irClass.classSerializer(
                compilerContext)!!.owner // internally generated serializer always declared inside serializable class

        val serialDescriptorGetter = serializerIrClass.getPropertyGetter(SERIAL_DESC_FIELD)!!.owner
        return irGet(serialDescriptorGetter.returnType, irGetObject(serializerIrClass), serialDescriptorGetter.symbol)
    }

    private fun IrBlockBodyBuilder.getParametrizedSerialDescriptorExpr(companionObject: IrClass,
            property: IrProperty): IrExpression {
    ...
    private fun createCachedDescriptorProperty(companionObject: IrClass): IrProperty {
        ...
        return companionObject.addValPropertyWithJvmField(serialDescIrType, CACHED_DESCRIPTOR_FIELD_NAME,
                DescriptorVisibilities.PUBLIC) {
            val serialDescVar = irTemporary(getInstantiateDescriptorExpr(), nameHint = "serialDesc")
    ...
    private fun IrBlockBodyBuilder.getInstantiateDescriptorExpr(): IrExpression {
        ...
        return irInvoke(null, serialClassDescImplCtor, irString(irClass.serialName()), irNull(),
                irInt(properties.serializableProperties.size))
    }

    private fun IrBlockBodyBuilder.getAddElementToDescriptorExpr(property: IrSerializableProperty,
            serialDescVar: IrVariable): IrExpression {
        return irInvoke(irGet(serialDescVar), addElementFun, irString(property.name), irBoolean(property.optional),
    ...
    private fun IrBlockBodyBuilder.generateSuperNonSerializableCall(superClass: IrClass) {
        ...
        val call = IrDelegatingConstructorCallImpl.fromSymbolOwner(startOffset, endOffset,
                compilerContext.irBuiltIns.unitType, ctorRef.symbol)
    ...
    private fun IrDelegatingConstructorCallImpl.insertTypeArgumentsForSuperClass(superClass: IrClass) {
        ...
        superTypeCallArguments?.forEachIndexed { index, irTypeArgument ->
            val argType = irTypeArgument as? IrTypeProjection ?: throw IllegalStateException(
                    "Star projection in immediate argument for supertype")
            putTypeArgument(index, argType.type)
        }
    }

    // returns offset in serializable properties array
    private fun IrBlockBodyBuilder.generateSuperSerializableCall(superClass: IrClass,
            allValueParameters: List<IrValueParameter>, propertiesStart: Int): Int {
        check(superClass.isInternalSerializable)
        val superCtorRef = superClass.findSerializableSyntheticConstructor() ?: error(
                "Class serializable internally should have special constructor with marker")
        ...
        val arguments = allValueParameters.subList(0, superSlots) + allValueParameters.subList(propertiesStart,
                propertiesStart + superProperties.size) + allValueParameters.last() // SerializationConstructorMarker
        val call = IrDelegatingConstructorCallImpl.fromSymbolOwner(startOffset, endOffset,
                compilerContext.irBuiltIns.unitType, superCtorRef)
    ...
    fun generateWriteSelfMethod(methodDescriptor: IrSimpleFunction) {
        addFunctionBody(methodDescriptor) { writeSelfFunction ->
            ...
            if (superClass.isInternalSerializable) {
                ...
                if (superWriteSelfF != null) { // Workaround for incorrect DeserializedClassDescriptor on JVM (see MemberDeserializer#getDispatchReceiverParameter):
                    ...
                    val args = mutableListOf<IrExpression>(irGet(objectToSerialize), irGet(localOutput),
                    ...
                    val parentWriteSelfSerializers = typeArgsForParent.map { arg ->
                        ...
                        serializerInstance(serial, compilerContext, arg.typeOrNull!!, genericIdx) { it, _ ->
                            irGet(writeSelfFunction.valueParameters[3 + it])
                        }!!
                    }
                    +irInvoke(null, superWriteSelfF.symbol, typeArgsForParent.map { it.typeOrNull!! },
            ...
            val cachedChildSerializerByIndex = createCacheableChildSerializersFactory(cachedChildSerializersProperty,
                    cachedChildSerializers.map { it != null }) { irClass.companionObject()!! }
    ...
    private fun generateSyntheticInternalConstructor() {
        ...
        if (irClass.shouldHaveSpecificSyntheticMethods {
                    serializerDescriptor.findPluginGeneratedMethod(LOAD, compilerContext.afterK2)
                }) {
    ...
    private fun generateSyntheticMethods() {
        val serializerDescriptor = irClass.classSerializer(compilerContext)?.owner ?: return
        if (irClass.shouldHaveSpecificSyntheticMethods {
                    serializerDescriptor.findPluginGeneratedMethod(SAVE, compilerContext.afterK2)
                }) {
    ...
    companion object {
        ...
        fun generate(irClass: IrClass, context: SerializationPluginContext) {
            if (irClass.isInternalSerializable) {
                ...
                if (serializableAnnotationIsUseless) error(
                        "@Serializable annotation on $irClass would be ignored because it is impossible to serialize it automatically. Provide serializer manually via e.g. companion object")
            }
        }
