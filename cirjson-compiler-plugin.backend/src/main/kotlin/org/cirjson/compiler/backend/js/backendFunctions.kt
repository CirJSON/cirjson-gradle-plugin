...
internal fun TranslationContext.buildFunction(descriptor: FunctionDescriptor,
        bodyGen: JsBlockBuilder.(JsFunction, TranslationContext) -> Unit): JsFunction {
...
internal fun propNotSeenTest(seenVar: JsNameRef, index: Int): JsBinaryOperation =
    JsAstUtils.equality(JsBinaryOperation(JsBinaryOperator.BIT_AND, seenVar, JsIntLiteral(1 shl (index % 32))),
            JsIntLiteral(0))
...
internal fun SerializerJsTranslator.serializerTower(property: SerializableProperty): JsExpression? {
    val nullableSerClass = context.translateQualifiedReference(
            property.module.getClassFromInternalSerializationPackage(SpecialBuiltins.nullableSerializer))
    val serializer = property.serializableWith?.toClassDescriptor
        ?: if (!property.type.isTypeParameter()) findTypeSerializerOrContext(property.module, property.type,
                property.descriptor.findPsi()) else null
    return serializerInstance(context, serializer, property.module, property.type, property.genericIndex)?.let { expr ->
        if (property.type.isMarkedNullable) JsNew(nullableSerClass, listOf(expr)) else expr
    }
}

internal fun AbstractSerialGenerator.serializerInstance(context: TranslationContext, serializerClass: ClassDescriptor?,
        module: ModuleDescriptor, kType: KotlinType, genericIndex: Int? = null,
        genericGetter: (Int, KotlinType) -> JsExpression = { it, _ ->
            JsNameRef(context.scope().declareName("${SerialEntityNames.typeArgPrefix}$it"), JsThisRef())
        }): JsExpression? {
    val nullableSerClass = context.translateQualifiedReference(
            module.getClassFromInternalSerializationPackage(SpecialBuiltins.nullableSerializer))
    ...
    fun instantiate(serializer: ClassDescriptor?, type: KotlinType): JsExpression? {
        val expr =
            serializerInstance(context, serializer, module, type, type.genericIndex, genericGetter) ?: return null
    ...
    var args = when {
        ...
        serializerClass.classId == enumSerializerId -> {
            ...
            val enumArgs = mutableListOf(JsStringLiteral(enumDescriptor.serialName()),
                    // EnumClass.values() invocation
                    JsInvocation(context.getInnerNameForDescriptor(
                            DescriptorUtils.getFunctionByName(enumDescriptor.staticScope, StandardNames.ENUM_VALUES))
                            .makeRef()))

            val packageScope = context.currentModule.getPackage(SerializationPackages.internalPackageFqName).memberScope
            val enumSerializerFactoryFunc = DescriptorUtils.getFunctionByNameOrNull(packageScope,
                    SerialEntityNames.ENUM_SERIALIZER_FACTORY_FUNC_NAME)
            val annotatedEnumSerializerFactoryFunc = DescriptorUtils.getFunctionByNameOrNull(packageScope,
            ...
            if (enumSerializerFactoryFunc != null && annotatedEnumSerializerFactoryFunc != null) {
                ...
                val factoryFunc = if (enumDescriptor.isEnumWithSerialInfoAnnotation()) {
                    ...
                    val entriesNames = enumEntries.map {
                        it.annotations.serialNameValue?.let { n -> JsStringLiteral(n) } ?: JsNullLiteral()
                    }
                    ...
                    val classAnnotationsConstructors =
                        enumDescriptor.annotationsWithArguments().map { (annotationClass, args, _) ->
                            val argExprs = args.map { arg ->
                                Translation.translateAsExpression(arg.getArgumentExpression()!!, context)
                            }
                            val classRef = context.translateQualifiedReference(annotationClass)
        ...
        serializerClass.classId == objectSerializerId -> listOf(JsStringLiteral(kType.serialName()),
        ...
        serializerClass.classId == sealedSerializerId -> mutableListOf<JsExpression>().apply {
            ...
            add(JsArrayLiteral(subclasses.map {
                ExpressionVisitor.getObjectKClass(context, it.toClassDescriptor!!)
            }))
            add(JsArrayLiteral(subSerializers.mapIndexed { i, serializer ->
                ...
                val expr = serializerInstance(context, serializer, module, type, type.genericIndex) { _, genericType ->
                    serializerInstance(context,
                            module.getClassFromSerializationPackage(SpecialBuiltins.polymorphicSerializer), module,
    ...
    if (serializerClass.classId == referenceArraySerializerId) args =
        listOf(ExpressionVisitor.getObjectKClass(context, kType.arguments[0].type.toClassDescriptor!!)) + args
    ...
    val ref = if (serializable?.declaredTypeParameters?.isNotEmpty() == true) {
        val desc = requireNotNull(findSerializerConstructorForTypeArgumentsSerializers(
                serializerClass)) { "Generated serializer does not have constructor with required number of arguments" }
        if (!desc.isPrimary) JsInvocation(context.getInnerReference(desc), args)
        else JsNew(context.getInnerReference(desc), args)
    } else {
...
fun TranslationContext.buildInitializersRemapping(forClass: KtPureClassOrObject,
        superClass: ClassDescriptor?): Map<PropertyDescriptor, KtExpression?> {
    val myMap = (forClass.bodyPropertiesDescriptorsMap(bindingContext())
            .mapValues { it.value.delegateExpressionOrInitializer } + forClass.primaryConstructorPropertiesDescriptorsMap(
            bindingContext()).mapValues { it.value.defaultValue })
