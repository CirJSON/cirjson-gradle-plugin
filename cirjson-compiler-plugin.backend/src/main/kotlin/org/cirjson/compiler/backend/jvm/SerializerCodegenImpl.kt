...
open class SerializerCodegenImpl(protected val codegen: ImplementationBodyCodegen, serializableClass: ClassDescriptor,
    ...
    override fun generateGenericFieldsAndConstructor(typedConstructorDescriptor: ClassConstructorDescriptor) {
        serializableDescriptor.declaredTypeParameters.forEachIndexed { i, _ ->
            codegen.v.newField(OtherOrigin(codegen.myClass.psiOrParent), Opcodes.ACC_PRIVATE or Opcodes.ACC_SYNTHETIC,
    ...
    private fun ExpressionCodegen.generateSerialDescriptor(descriptorVar: Int, isStatic: Boolean) = with(v) {
        ...
        addSyntheticAnnotationsToDescriptor(descriptorVar, serializableDescriptor,
    ...
    protected open fun ExpressionCodegen.instantiateNewDescriptor(isStatic: Boolean) = with(v) {
        ...
        if (isStatic) {
            ...
            if (isGeneratedSerializer) StackValue.singleton(serializerDescriptor, codegen.typeMapper)
                    .put(generatedSerializerType, this)
            else aconst(null)
        } else {
            load(0, serializerAsmType)
        }
        aconst(serializableProperties.size)
        invokespecial(descImplType.internalName, "<init>",
                "(Ljava/lang/String;${generatedSerializerType.descriptor}I)V", false)
    }
    ...
    protected fun ExpressionCodegen.addSyntheticAnnotationsToDescriptor(descriptorVar: Int, annotated: Annotated,
            functionToCall: String) = with(v) {
        for ((annotationClass, args, consParams) in annotated.annotationsWithArguments()) {
            if (args.size != consParams.size) throw IllegalArgumentException(
            ...
            invokevirtual(descImplType.internalName, functionToCall, "(Ljava/lang/annotation/Annotation;)V", false)
        }
    }

    override fun generateSerialDesc() {
        var flags = Opcodes.ACC_PRIVATE or Opcodes.ACC_FINAL or Opcodes.ACC_SYNTHETIC
        if (staticDescriptor) flags = flags or Opcodes.ACC_STATIC
        codegen.v.newField(OtherOrigin(codegen.myClass.psiOrParent), flags, serialDescField, descType.descriptor, null,
                null)
    ...
    protected fun InstructionAdapter.stackSerialClassDesc(classDescVar: Int?) {
        if (staticDescriptor) getstatic(serializerAsmType.internalName, serialDescField, descType.descriptor)
        else {
            load(0, serializerAsmType)
    ...
    override fun generateChildSerializersGetter(function: FunctionDescriptor) {
        codegen.generateMethod(function) { _, expressionCodegen ->
            ...
            for (i in 0 until size) {
                ...
                assert(stackValueSerializerInstanceFromSerializerWithoutSti(expressionCodegen, codegen, prop,
                        this@SerializerCodegenImpl)) { "Property ${prop.name} must have serializer" }
    ...
    override fun generateSave(function: FunctionDescriptor) {
        codegen.generateMethod(function) { signature, expressionCodegen ->
            ...
            invokeinterface(encoderType.internalName, CallingConventions.begin,
                    "(" + descType.descriptor + ")" + kOutputType.descriptor)
            ...
            if (serializableDescriptor.isInternalSerializable) {
                ...
                serializableDescriptor.declaredTypeParameters.forEachIndexed { i, _ ->
                    ...
                    getfield(codegen.typeMapper.mapClass(codegen.descriptor).internalName, "$typeArgPrefix$i",
                            kSerializerType.descriptor)
                    sig.append(kSerializerType.descriptor)
                }
                sig.append(")V")
                invokestatic(objType.internalName, SerialEntityNames.WRITE_SELF_NAME.asString(), sig.toString(), false)
            } else {
                ...
                for (index in labeledProperties.indices) {
                    ...
                    genKOutputMethodCall(property, codegen, expressionCodegen, objType, objVar,
                            generator = this@SerializerCodegenImpl)
                }
            }
            // output.writeEnd(classDesc)
            load(outputVar, kOutputType)
            load(descVar, descType)
            invokeinterface(kOutputType.internalName, CallingConventions.end, "(" + descType.descriptor + ")V")
            // return
            areturn(Type.VOID_TYPE)
        }
    }
    ...
    override fun generateLoad(function: FunctionDescriptor) {
        codegen.generateMethod(function) { _, expressionCodegen ->
            ...
            load(descVar, descType)
            invokeinterface(decoderType.internalName, CallingConventions.begin,
                    "(" + descType.descriptor + ")" + kInputType.descriptor)
            ...
            invokeinterface(kInputType.internalName, CallingConventions.decodeSequentially, "()Z")
            ...
            load(inputVar, kInputType)
            load(descVar, descType)
            invokeinterface(kInputType.internalName, CallingConventions.decodeElementIndex,
            ...
            load(inputVar, kInputType)
            load(descVar, descType)
            invokeinterface(kInputType.internalName, CallingConventions.end, "(" + descType.descriptor + ")V")
            if (!serializableDescriptor.isInternalSerializable) {
                ...
                for ((i, property) in properties.serializableConstructorProperties.withIndex()) {
                    if (property.optional || property.transient) {
                        if (!property.isConstructorParameterWithDefault) throw CompilationException(
            ...
            dup()
            val constructorDesc =
                if (serializableDescriptor.isInternalSerializable) buildInternalConstructorDesc(propsStartVar,
                        bitMaskBase, codegen, properties.serializableProperties)
            ...
            if (!serializableDescriptor.isInternalSerializable && !properties.serializableStandaloneProperties.isEmpty()) {
                ...
                propVar = propsStartVar + properties.serializableConstructorProperties.map {
                    codegen.typeMapper.mapType(it.type).size
                }.sum()
    ...
    private fun InstructionAdapter.callReadProperty(expressionCodegen: ExpressionCodegen,
            property: SerializableProperty, propertyType: Type, index: Int, inputVar: Int, descriptorVar: Int,
            propertyVar: Int) {
        ...
        val useSerializer =
            stackValueSerializerInstanceFromSerializer(expressionCodegen, codegen, sti, this@SerializerCodegenImpl)
        ...
        fun produceCall(isUpdatable: Boolean) {
            invokeinterface(kInputType.internalName,
                    ...
                    "(" + descType.descriptor + "I" + (if (useSerializer) kSerialLoaderType.descriptor else "") + (if (unknownSer) AsmTypes.K_CLASS_TYPE.descriptor else "") + (if (isUpdatable) sti.type.descriptor else "") + ")" + (sti.type.descriptor))
        }
    ...
    private fun InstructionAdapter.buildExternalConstructorDesc(propsStartVar: Int, bitMaskBase: Int): String {
        ...
        if (!properties.primaryConstructorWithDefaults) {
            ...
            val cnt = properties.serializableConstructorProperties.size.coerceAtMost(
                    32) //only 32 default values are supported
    ...
    private fun InstructionAdapter.genSetSerializableStandaloneProperties(expressionCodegen: ExpressionCodegen,
            propVarStart: Int, resultVar: Int, bitMaskPos: (Int) -> Int) {
        ...
        for ((index, property) in properties.serializableStandaloneProperties.withIndex()) {
            ...
            propVar += propertyType.size
            if (property.optional) visitLabel(nextLabel)
        }
    }
