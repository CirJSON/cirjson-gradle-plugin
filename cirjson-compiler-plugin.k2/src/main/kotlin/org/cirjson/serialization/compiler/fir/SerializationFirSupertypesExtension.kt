...
import org.cirjson.serialization.compiler.fir.FirSerializationPredicates.annotatedWithSerializableOrMeta
import org.cirjson.serialization.compiler.fir.FirSerializationPredicates.serializerFor
...
class SerializationFirSupertypesExtension(session: FirSession) : FirSupertypeGenerationExtension(session) {
    ...
    override fun needTransformSupertypes(declaration: FirClassLikeDeclaration): Boolean =
        session.predicateBasedProvider.matches(serializerFor, declaration) || isSerializableObjectAndNeedsFactory(
                declaration) || isCompanionAndNeedsFactory(declaration)
    ...
    private fun isSerializableObjectAndNeedsFactory(declaration: FirClassLikeDeclaration): Boolean = with(session) {
        ...
        return declaration is FirClass && declaration.classKind.isObject && session.predicateBasedProvider.matches(
                annotatedWithSerializableOrMeta, declaration)
    }
    ...
    private fun isCompanionAndNeedsFactory(declaration: FirClassLikeDeclaration): Boolean = with(session) {
        ...
        return session.predicateBasedProvider.matches(annotatedWithSerializableOrMeta,
                parentSymbol) && parentSymbol.companionNeedsSerializerFactory
    }
    ...
    override fun computeAdditionalSupertypes(
        ...
        val generatedSerializerClassId =
        ...
        return when {
            ...
            isSerializableObjectAndNeedsFactory(classLikeDeclaration) || isCompanionAndNeedsFactory(
                    classLikeDeclaration) -> {
                val serializerFactoryClassId = ClassId(SerializationPackages.internalPackageFqName,
