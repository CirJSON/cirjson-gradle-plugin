...
import org.cirjson.compiler.resolve.SerialEntityNames.SERIALIZER_FACTORY_INTERFACE_NAME
import org.cirjson.compiler.resolve.SerializationPackages
import org.cirjson.compiler.resolve.SerializersClassIds.generatedSerializerId
import org.cirjson.compiler.resolve.SerializersClassIds.kSerializerId
...
class SerializationFirResolveExtension(session: FirSession) : FirDeclarationGenerationExtension(session) {
    ...
    internal val runtimeHasEnumSerializerFactory by lazy {
        val hasFactory = session.symbolProvider.getTopLevelCallableSymbols(SerializationPackages.internalPackageFqName,
                SerialEntityNames.ENUM_SERIALIZER_FACTORY_FUNC_NAME).isNotEmpty()
        val hasAnnotatedFactory =
            session.symbolProvider.getTopLevelCallableSymbols(SerializationPackages.internalPackageFqName,
                    SerialEntityNames.ANNOTATED_ENUM_SERIALIZER_FACTORY_FUNC_NAME).isNotEmpty()
        hasFactory && hasAnnotatedFactory
    }

    override fun getNestedClassifiersNames(classSymbol: FirClassSymbol<*>,
            context: NestedClassGenerationContext): Set<Name> {
        val result = mutableSetOf<Name>()
        with(session) {
            if (classSymbol.shouldHaveGeneratedMethodsInCompanion && !classSymbol.isSerializableObject) result += SpecialNames.DEFAULT_NAME_FOR_COMPANION_OBJECT
            if (classSymbol.shouldHaveGeneratedSerializer && !classSymbol.isInternallySerializableObject) result += SerialEntityNames.SERIALIZER_CLASS_NAME
        }

        return result
    }

    override fun generateNestedClassLikeDeclaration(owner: FirClassSymbol<*>, name: Name,
            context: NestedClassGenerationContext): FirClassLikeSymbol<*>? {
        if (owner !is FirRegularClassSymbol) return null
        if (!session.predicateBasedProvider.matches(FirSerializationPredicates.annotatedWithSerializableOrMeta,
                        owner)) return null
        return when (name) {
    ...
    @OptIn(SymbolInternals::class) private fun <T> getFromSupertype(callableId: CallableId, owner: FirClassSymbol<*>,
            extractor: (FirTypeScope) -> List<T>): T {
        val scopeSession = ScopeSession()
        val scopes = lookupSuperTypes(owner, lookupInterfaces = true, deep = false,
                useSiteSession = session).mapNotNull { useSiteSuperType ->
        ...
        return targets.singleOrNull() ?: error(
                "Zero or multiple overrides found for ${callableId.callableName} in $owner")
    }

    @OptIn(SymbolInternals::class) override fun generateFunctions(callableId: CallableId,
            context: MemberGenerationContext?): List<FirNamedFunctionSymbol> {
        ...
        if (callableId.callableName == SerialEntityNames.SERIALIZER_PROVIDER_NAME) {
            val serializableClass = if (owner.isCompanion) {
                val containingSymbol =
                    owner.getContainingDeclarationSymbol(session) as? FirClassSymbol<*> ?: return emptyList()
            ...
            val serializableGetterInCompanion =
                generateSerializerGetterInCompanion(owner, serializableClass, callableId)
            val serializableGetterFromFactory =
                runIf(with(session) { serializableClass.companionNeedsSerializerFactory }) {
        ...
        if (callableId.callableName !in setOf(SpecialNames.INIT, SerialEntityNames.SAVE_NAME,
                        SerialEntityNames.LOAD_NAME, SerialEntityNames.CHILD_SERIALIZERS_GETTER,
                        SerialEntityNames.TYPE_PARAMS_SERIALIZERS_GETTER)) return emptyList()
    ...
    private fun generateSerializerFactoryVararg(owner: FirClassSymbol<*>, callableId: CallableId,
            original: FirSimpleFunction): FirNamedFunctionSymbol =
        createMemberFunction(owner, SerializationPluginKey, callableId.callableName, original.returnTypeRef.coneType) {
            ...
            valueParameter(vpo.name, vpo.returnTypeRef.coneType, vpo.isCrossinline, vpo.isNoinline, vpo.isVararg,
                    vpo.defaultValue != null)
        }.apply {
            excludeFromJsExport()
        }.symbol

    private fun generateSerializerGetterInCompanion(owner: FirClassSymbol<*>,
            serializableClassSymbol: FirClassSymbol<*>, callableId: CallableId): FirNamedFunctionSymbol {
        val function = createMemberFunction(owner, SerializationPluginKey, callableId.callableName,
            ...
            serializableClassSymbol.typeParameterSymbols.forEachIndexed { i, typeParameterSymbol ->
                ...
                valueParameter(Name.identifier("${SerialEntityNames.typeArgPrefix}$i"), { typeParameters ->
    ...
    override fun generateProperties(callableId: CallableId,
            context: MemberGenerationContext?): List<FirPropertySymbol> {
        val owner = context?.owner ?: return emptyList()
        if (!owner.isSerializer) return emptyList()
        ...
        val property =
            createMemberProperty(owner, SerializationPluginKey, callableId.callableName, target.resolvedReturnType)
        ...
        if (owner.name == SerialEntityNames.SERIALIZER_CLASS_NAME && owner.typeParameterSymbols.isNotEmpty()) {
            result += createConstructor(owner, SerializationPluginKey) {
                ...
                owner.typeParameterSymbols.forEachIndexed { i, typeParam ->
                    valueParameter(name = Name.identifier("${SerialEntityNames.typeArgPrefix}$i"),
    ...
    private fun generateSerializerImplClass(owner: FirRegularClassSymbol): FirClassLikeSymbol<*> {
        ...
        val serializerFirClass =
            createNestedClass(owner, SerialEntityNames.SERIALIZER_CLASS_NAME, SerializationPluginKey, serializerKind) {
                ...
                superType { typeParameters ->
                    generatedSerializerId.constructClassLikeType(
                            arrayOf(owner.constructType(typeParameters.map { it.toConeType() }.toTypedArray(),
    ...
    private fun generateCompanionDeclaration(owner: FirRegularClassSymbol): FirRegularClassSymbol? {
        ...
        val companion = createCompanionObject(owner, SerializationPluginKey) {
            if (with(session) { owner.companionNeedsSerializerFactory }) {
                val serializerFactoryClassId =
                    ClassId(SerializationPackages.internalPackageFqName, SERIALIZER_FACTORY_INTERFACE_NAME)
    ...
    override fun FirDeclarationPredicateRegistrar.registerPredicates() {
        register(FirSerializationPredicates.annotatedWithSerializableOrMeta,
