...
class SerializationFirDeclarationsForMetadataProvider(session: FirSession) :
    FirDeclarationsForMetadataProviderExtension(session) {
    ...
    private fun generateDeserializationConstructor(klass: FirClass): FirDeclaration =
        createConstructor(klass.symbol, SerializationPluginKey, isPrimary = false) {
            ...
            val serializableProperties = session.serializablePropertiesProvider.getSerializablePropertiesForClass(
                    klass.symbol).serializableProperties
            ...
            serializableProperties.forEach { prop ->
                valueParameter(prop.originalDescriptorName,
                        prop.propertySymbol.resolvedReturnType.makeNullableIfNotPrimitive(session.typeContext))
            }
            val markerType = ClassId(SerializationPackages.internalPackageFqName,
                    SerialEntityNames.SERIAL_CTOR_MARKER_NAME).constructClassLikeType(emptyArray(), isNullable = true)
    ...
    private fun generateWriteSelf(klass: FirClass): FirDeclaration? {
        ...
        return createMemberFunction(klass.symbol, SerializationPluginKey, SerialEntityNames.WRITE_SELF_NAME,
            ...
            klass.typeParameters.forEachIndexed { i, _ ->
                valueParameter(Name.identifier("${SerialEntityNames.typeArgPrefix}$i"), { functionTps ->
                    SerializersClassIds.kSerializerId.constructClassLikeType(arrayOf(functionTps[i].toConeType()),
                            false)
                })
            }
    ...
    private fun createJvmStaticAnnotation(): FirAnnotation? {
        val jvmStatic = session.symbolProvider.getClassLikeSymbolByClassId(
                StandardClassIds.Annotations.JvmStatic) as? FirRegularClassSymbol ?: return null
        val jvmStaticCtor = jvmStatic.declarationSymbols.firstIsInstanceOrNull<FirConstructorSymbol>() ?: return null
